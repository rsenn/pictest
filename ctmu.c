...
#if defined(CTMUGETVOLTAGE) || defined(CTMUGETCURRENT) || defined(CTMUGETCAPACITANCE) || defined(CTMUGETTIME)

float
CTMU_getVoltage(u8 channel) {
  u8 j;
  u16 VTot = 0, Vread = 0;
  float Vavg, Vcal;
...
float
CTMU_getCapacitance(u8 channel) {
  float Vcal = 0;
  float CTMUISrc = 0; // current value
