...
bool
CIntelHexRec::InitFromString(string str) {
  string strVal;
  char* pEnds;
  unsigned long uVal;
  int i;
...
unsigned char
CIntelHexRec::CalcCrc() {
  unsigned long Res = 0;
  Res += m_Size;
  Res += HIBYTE(m_Addr);
  Res += LOBYTE(m_Addr);
  Res += m_Type;
  for(int i = 0; i < m_Size; i++) Res += m_Data[i];
  Res = 0x100 - (Res & 0xFF);
  return (unsigned char)Res;
}

string
CIntelHexRec::GetString() {
  string pc = "";
...
void
CIntelHexRec::InitData(unsigned char Size, unsigned short Addr, unsigned char* pData) {
  eAssert(pData);
  m_Type = cHexTypeData;
  m_Size = Size;
  m_Addr = Addr;
...
void
CIntelHexRec::InitExtAddr(unsigned short ExtAddr) {
