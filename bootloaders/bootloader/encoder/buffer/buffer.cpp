...
Buffer::Buffer(size_t size, unsigned char fill /* = 0xFF */) : _data(NULL), _size(0) { resize(size, fill); }

void
Buffer::fill(unsigned char val) {
...
size_t
Buffer::read(unsigned char* buf, size_t buf_size, size_t addr) {
  eAssert(_data != NULL);
  eAssert(addr < _size);
  size_t size;
...
void
Buffer::write(unsigned char* buf, size_t buf_size, size_t addr) {
  eAssert(buf != NULL);
...
void
Buffer::resize(size_t size, unsigned char fill /* = 0xFF */) {
  if(_data != NULL) {
    delete[] _data;
    _data = NULL;
  }
...
void
Buffer::swapBytes(unsigned long word_size) {
  size_t current, i, j;
  unsigned char temp;
