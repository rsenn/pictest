...
IntelHexImage::IntelHexImage(string name)
    : _name(name), _file(NULL), _size(0), _linAddr(1) // LOW 16 bit MUST be nonzero at first time write() called
{}

IntelHexImage::~IntelHexImage() { eAssert(_file == NULL); }

void
IntelHexImage::setWriteAddress(size_t addr) {
  _writeAddr = addr;
}

void
IntelHexImage::open(bool read) {
...
size_t
IntelHexImage::read(unsigned char* buffer, size_t bufSize, size_t* addr) {
  ...
  while(_readCurrent >= _record.m_Size) { // read from file
...
void
IntelHexImage::write(unsigned char* buffer, size_t bufSize) {
  size_t address, offset, current;
  string str;
  CIntelHexRec record;
  for(offset = 0; offset < bufSize; offset += current) {
    address = _writeAddr + offset;
    if((_linAddr & 0xFFFFU) || ((_linAddr & 0xFFFF0000U) != (_writeAddr & 0xFFFF0000U)) ||
       ((address != 0) && (address % 0x10000 == 0))) {
