...
FragBuffer::FragBuffer(unsigned char fill) : _fillVal(fill) {}

FragBuffer::~FragBuffer() { clear(); }

void
FragBuffer::fill(unsigned char val) {
  _fillVal = val;
  for(FragBuffer::iterator it = begin(); it != end(); it++) (*it).second = _fillVal;
}
...
size_t
FragBuffer::read_data(unsigned char* buf, size_t buf_size, size_t addr) {
...
void
FragBuffer::write(unsigned char* buf, size_t buf_size, size_t addr) {
  for(size_t i = 0; i < buf_size; i++) (*this)[i + addr] = buf[i];
}

void
FragBuffer::swapBytes(const size_t word_size) {
...
size_t
FragBuffer::address(const FragBuffer::iterator& it) {
  return (*it).first;
}

unsigned char
FragBuffer::value(const FragBuffer::iterator& it) {
  return (*it).second;
}
