...
void
AddTraceType(char* pMessage, const ETraceType TraceType) {
...
void
AddFileLine(char* pMessage, const char* FileName, const int LineNumber) {
  char TraceMessage[TRACE_MESSAGE_SIZE];
  _snprintf(TraceMessage, TRACE_MESSAGE_SIZE, "  File: %s, Line: %d", FileName, LineNumber);
  strncat(pMessage, TraceMessage, TRACE_MESSAGE_SIZE);
}

void
AddDateTime(char* pMessage) {
  SYSTEMTIME t;
  GetLocalTime(&t);
  char TraceMessage[TRACE_MESSAGE_SIZE];

  _snprintf(TraceMessage,
            TRACE_MESSAGE_SIZE,
...
void
DebuggerTrace(const ETraceType TraceType, const char* FileName, const int LineNumber, char* TraceMessage) {
...
void
FileTrace(const ETraceType TraceType, const char* FileName, const int LineNumber, char* TraceMessage) {
  AddTraceType(TraceMessage, TraceType);
  AddFileLine(TraceMessage, FileName, LineNumber);
...
void
OutputTrace(const ETraceType TraceType,
...
void
imp_Trace(const ETraceType TraceType,
          const ETraceOutput TraceOutput,
          const char* FileName,
          const int LineNumber,
          const char* Str,
...
void
imp_eAssert(const char* TraceString,
