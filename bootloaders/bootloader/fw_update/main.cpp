...
void
save(size_t start, Buffer::Iterator* iter, Image* image, const Parameters& params) {
  try {
...
size_t
load(Buffer* buffer, PicBootloader* device, const PicBootloader::MemoryType memory, const Parameters& params) {
  Image* image = NULL;
  const size_t dataSize = 0x100;
  unsigned char data[dataSize];
  size_t max = 0;
...
void
erase(PicBootloader* device, const Parameters& params) {
...
void
read(PicBootloader* device, const Parameters& params) {
  Image* image = NULL;
...
void
verify(PicBootloader* device, const Parameters& params) {
  PicBootloader::MemoryType memory = device->memory_type(params);
...
void
write(PicBootloader* device, const Parameters& params) {
  PicBootloader::MemoryType memory = device->memory_type(params);
...
void
getversion(PicBootloader* device, const Parameters& params) {
  boot_rsp response;

  device->getversion(&response, params);
  cout << "BootLoader Version"
       << ": " << (unsigned int)response.get_fw_ver.major << "." << (unsigned int)response.get_fw_ver.minor << "."
       << (unsigned int)response.get_fw_ver.sub_minor << endl;
}
  ...
  try {
    ...
    if(device.memory_type(params) == PicBootloader::MEM_ALL) {
      ...
      if(params.contain(ARG_DATA_SIZE))
        cerr << "WARNING: argument \"" << params[ARG_DATA_SIZE].argument() << "\" ignored for memory type = all"
             << endl;
