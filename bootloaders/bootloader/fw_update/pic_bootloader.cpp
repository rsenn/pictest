...
PicBootloader::MemoryType
PicBootloader::memory_type(const Parameters& params) {
  if(params.contain(ARG_MEM_TYPE)) {
    if(params[ARG_MEM_TYPE].value() == "flash")
      return MEM_FLASH;
    else if(params[ARG_MEM_TYPE].value() == "eeprom")
      return MEM_EEPROM;
    else if(params[ARG_MEM_TYPE].value() == "id")
      return MEM_ID;
    else if(params[ARG_MEM_TYPE].value() == "all")
      return MEM_ALL;
    else {
      list<string> l;
      l.push_back("flash");
...
PicBootloader::MemoryType
PicBootloader::memory_type(std::string memory) {
  if(memory == "flash")
    return MEM_FLASH;
  else if(memory == "eeprom")
    return MEM_EEPROM;
  else if(memory == "id")
    return MEM_ID;
  else if(memory == "all")
...
void
PicBootloader::mem_range(MemoryType memory, size_t* start, size_t* size) {
  switch(memory) {
    case MEM_FLASH:
...
void
PicBootloader::mem_border(PicBootloader::MemoryType memory,
                          size_t max_size,
                          size_t* start,
                          size_t* end,
                          size_t* size,
...
void
PicBootloader::erase(string memory, const Parameters& params) {
...
void
PicBootloader::reset(const Parameters& params) {
  boot_cmd command;
  memset(&command, 0, sizeof(boot_cmd));
...
void
PicBootloader::verify(Buffer::Iterator* buffer, string memory, const Parameters& params) {
  PicBootloader::MemoryType mem = memory_type(memory);
  unsigned char cmd;
...
void
PicBootloader::read(Buffer::Iterator* buffer, string memory, const Parameters& params) {
  PicBootloader::MemoryType mem = memory_type(memory);
  size_t size, address, def_addr, def_size;
  unsigned char cmd;
  boot_cmd command;
...
void
PicBootloader::program(Buffer::Iterator* buffer, string memory, const Parameters& params) {
  PicBootloader::MemoryType mem = memory_type(memory);
  size_t size, address, def_addr, def_size;
  unsigned char cmd;
  boot_cmd command;
...
void
PicBootloader::getversion(boot_rsp* response, const Parameters& params) {
...
PicBootloader::PicBootloader() : m_hDevice(INVALID_HANDLE_VALUE), command_id(0) {}
...
void
PicBootloader::open(const Parameters& params) {
  eAssert(m_hDevice == INVALID_HANDLE_VALUE);
  try {
#ifdef BUILD_WIN32
    GUID HidGuid;
    ...
    for(unsigned long DevIndex = 0; true; DevIndex++) {
      ...
      if(isValidDevice(DevicePath, params)) {
        // Valid device found
        m_hDevice = CreateFile(DevicePath.c_str(),
...
#ifdef BUILD_WIN32
bool
PicBootloader::isValidDevice(string DevicePath, const Parameters& params) {
...
#else
bool
PicBootloader::isValidDevice(struct usb_device* dev, const Parameters& params) {
  if((dev->descriptor.idVendor == params[ARG_VID].ushortValue()) &&
     (dev->descriptor.idProduct == params[ARG_PID].ushortValue()))
    return true;
...
void
PicBootloader::send_command(boot_cmd* pOut) {
#ifdef BUILD_WIN32
...
void
PicBootloader::transaction(boot_cmd* pOut, boot_rsp* pIn) {
#ifdef BUILD_WIN32
...
void
PicBootloader::prepare_cmd(PicBootloader::MemoryType memory,
                           bool rd,
                           unsigned char* cmd,
                           size_t* def_addr,
                           size_t* def_size,
