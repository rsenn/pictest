...
// Global Variables
// *****************************************************************************
// DEFAULT_ROLE needs to be configured as ROLE_HOST or ROLE_DEVICE if not using a Micro AB OTG cable, see usb_config.h
// for configuring
BYTE CurrentRole, DefaultRole;
WORD HNPTimeOut, SRPTimeOut;
BOOL HNPActive = 0, SRPActive = 0, SRPReady = 0, HNPEnable = 0, HNPSupport = 0, HNPAltSupport = 0, HNPTimeOutFlag = 0,
     SRPTimeOutFlag = 0;
BOOL RoleSwitch = 0;
...
// DOM-IGNORE-END
void
USB_OTGEventHandler(BYTE address, BYTE event, void* data, DWORD size) {
  switch(event) {
    ...
    case OTG_EVENT_HNP_FAILED: // HNP Fail Condition, Caused by device not responding When HNP Is Enabled And A Suspend
                               // is Issued or Received(T1MSECIF)
      if(DefaultRole == ROLE_HOST) {
        // Initialize Host Stack
        USBOTGInitializeHostStack();
    ...
    case OTG_EVENT_SRP_FAILED: // SRP Fail Condition, Caused by A or B side not Connecting on a SRP Transition
                               // (T1MSECIF)
      if(DefaultRole == ROLE_HOST) {
        // Clear SRP Ready
        SRPReady = 0;
