...
#ifndef USB_ENABLE_TRANSFER_EVENT
#define _USBHostMSD_SetNextState()                                                                                     \
  { deviceInfoMSD[i].state = (deviceInfoMSD[i].state & STATE_MASK) + NEXT_STATE; }
#define _USBHostMSD_SetNextSubState()                                                                                  \
  { deviceInfoMSD[i].state += NEXT_SUBSTATE; }
...
uint8_t
USBHostMSDDeviceStatus(uint8_t deviceAddress) {
  uint8_t i;
  uint8_t status;
...
uint8_t
USBHostMSDResetDevice(uint8_t deviceAddress) {
  uint8_t i;
...
#else
  if((deviceInfoMSD[i].state == STATE_RUNNING) || (deviceInfoMSD[i].state == STATE_HOLDING))
#endif
  {
          ...
          switch(deviceInfoMSD[i].state & SUBSTATE_MASK) {
            ...
            case SUBSTATE_SEND_GET_MAX_LUN:
              ...
              if(!USBHostIssueDeviceRequest(deviceInfoMSD[i].deviceAddress,
                                            ...
                                            USB_MSD_GET_MAX_LUN,
                                            0,
                                            deviceInfoMSD[i].interface,
                                            1,
                                            deviceInfoMSD[i].block.data,
                                            USB_DEVICE_REQUEST_GET,
                                            deviceInfoMSD[i].clientDriverID)) {
                _USBHostMSD_SetNextSubState();
              } else {
                // We'll try again.
              }
            ...
            case SUBSTATE_GET_MAX_LUN_COMPLETE:
              ...
              if(usbMediaInterfaceTable.Initialize(deviceInfoMSD[i].deviceAddress, usbMediaInterfaceTable.flags, 0)) {
                usbMediaInterfaceTable.EventHandler(deviceInfoMSD[i].deviceAddress,
                                                    EVENT_MSD_MAX_LUN,
                                                    &(deviceInfoMSD[i].maxLUN),
                                                    1);
          ...
          switch(deviceInfoMSD[i].state & SUBSTATE_MASK) {
...
#endif
              errorCode = USBHostWrite(deviceInfoMSD[i].deviceAddress,
                                       deviceInfoMSD[i].endpointOUT,
                                       deviceInfoMSD[i].block.data,
                                       CBW_SIZE);
              if(errorCode) {
            ...
            case SUBSTATE_CBW_WAIT:
              if(USBHostTransferIsComplete(
                     deviceInfoMSD[i].deviceAddress, deviceInfoMSD[i].endpointOUT, &errorCode, &byteCount)) {
                if(errorCode) {
#ifdef DEBUG_MODE
            ...
            case SUBSTATE_TRANSFER_DATA:
              ...
              if(deviceInfoMSD[i].userDataLength == 0) {
                ...
                {
                  errorCode = USBHostWrite(deviceInfoMSD[i].deviceAddress,
                                           deviceInfoMSD[i].endpointDATA,
                                           deviceInfoMSD[i].userData,
                                           deviceInfoMSD[i].userDataLength);
                } else {
                  errorCode = USBHostRead(deviceInfoMSD[i].deviceAddress,
                                          deviceInfoMSD[i].endpointDATA,
                                          deviceInfoMSD[i].userData,
                                          deviceInfoMSD[i].userDataLength);
                }
            ...
            case SUBSTATE_TRANSFER_WAIT:
              if(USBHostTransferIsComplete(
                     deviceInfoMSD[i].deviceAddress, deviceInfoMSD[i].endpointDATA, &errorCode, &byteCount)) {
                if(errorCode) {
...
// Clear the stall, then try to get the CSW.
            ...
            case SUBSTATE_CSW_WAIT:
              if(USBHostTransferIsComplete(
                     deviceInfoMSD[i].deviceAddress, deviceInfoMSD[i].endpointIN, &errorCode, &byteCount)) {
#ifdef DEBUG_MODE
                ...
                if(errorCode) {
                  ...
                  deviceInfoMSD[i].bytesTransferred =
                      deviceInfoMSD[i].userDataLength - deviceInfoMSD[i].block.csw.dCSWDataResidue;
        ...
        case STATE_MSD_RESET_RECOVERY:
          switch(deviceInfoMSD[i].state & SUBSTATE_MASK) {
            case SUBSTATE_SEND_RESET:
              errorCode = USBHostIssueDeviceRequest(deviceInfoMSD[i].deviceAddress,
...
void
USBHostMSDTerminateTransfer(uint8_t deviceAddress) {
  uint8_t i;
...
#else
  if((deviceInfoMSD[i].state == STATE_RUNNING) || (deviceInfoMSD[i].state == STATE_CBW_WAIT) ||
     (deviceInfoMSD[i].state == STATE_TRANSFER_WAIT) || (deviceInfoMSD[i].state == STATE_CSW_WAIT))
#endif
  {
...
uint8_t
USBHostMSDTransfer(uint8_t deviceAddress,
                   uint8_t deviceLUN,
                   uint8_t direction,
...
bool
USBHostMSDTransferIsComplete(uint8_t deviceAddress, uint8_t* errorCode, uint32_t* byteCount) {
  uint8_t i;
...
bool
USBHostMSDInitialize(uint8_t address, uint32_t flags, uint8_t clientDriverID) {
  uint8_t* descriptor;
  uint8_t device;
        ...
        if(((descriptor[i + 6] == DEVICE_SUBCLASS_SCSI) || (descriptor[i + 6] == DEVICE_SUBCLASS_REMOVABLE)) &&
          ...
          if((endpointIN != 0) && (endpointOUT != 0)) {
            ...
            errorCode = USBHostIssueDeviceRequest(deviceInfoMSD[device].deviceAddress,
                                                  ...
                                                  USB_MSD_GET_MAX_LUN,
                                                  0,
                                                  deviceInfoMSD[device].interface,
                                                  1,
                                                  deviceInfoMSD[device].block.data,
                                                  USB_DEVICE_REQUEST_GET,
                                                  deviceInfoMSD[device].clientDriverID);
            if(errorCode) {
#ifdef DEBUG_MODE
...
bool
USBHostMSDEventHandler(uint8_t address, USB_EVENT event, void* data, uint32_t size) {
...
// Skip to getting the CSW
        ...
        case STATE_CSW_WAIT:
          ...
          if(((HOST_TRANSFER_DATA*)data)->bErrorCode) {
            ...
            deviceInfoMSD[i].bytesTransferred =
                deviceInfoMSD[i].userDataLength - ((USB_MSD_CSW*)(deviceInfoMSD[i].block.data))->dCSWDataResidue;
...
void
_USBHostMSD_ResetStateJump(uint8_t i) {
#ifdef USB_ENABLE_TRANSFER_EVENT
  uint8_t errorCode;
#endif
...
#else
    errorCode =
        USBHostIssueDeviceRequest(deviceInfoMSD[i].deviceAddress,
