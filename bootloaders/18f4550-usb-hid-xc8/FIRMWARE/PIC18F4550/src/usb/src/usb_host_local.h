...
#define USB_ENDPOINT_CONTROL_INTERRUPT                                                                                 \
  (UEP_NO_SETUP_TRANSFERS | UEP_RX_ENABLE | UEP_TX_ENABLE | UEP_HANDSHAKE_ENABLE)         //
...
#define USB_RESET_RECOVERY_TIME                                                                                        \
  (100 + 1) // RESET recovery time - Changed to 100 ms from 10ms.  Some devices take longer.
#else
...
typedef struct _USB_BUS_INFO {
  ...
  //    volatile uint32_t      dBytesSentInFrame;                  // The number of bytes sent during the current frame.
  //    Isochronous use only.
...
typedef struct _USB_DEVICE_INFO {
  ...
  USB_CONFIGURATION*
      pConfigurationDescriptorList;   // Pointer to the list of Cnfiguration Descriptors of the attached device.
...
#define _USB_InitErrorCounters()                                                                                       \
  { numCommandTries = USB_NUM_COMMAND_TRIES; }
#define _USB_SetDATA01(x)                                                                                              \
  { pCurrentEndpoint->status.bfNextDATA01 = x; }
#define _USB_SetErrorCode(x)                                                                                           \
  { usbDeviceInfo.errorCode = x; }
#define _USB_SetHoldState()                                                                                            \
  { usbHostState = STATE_HOLDING; }
#define _USB_SetNextState()                                                                                            \
  { usbHostState = (usbHostState & STATE_MASK) + NEXT_STATE; }
#define _USB_SetNextSubState()                                                                                         \
  { usbHostState = (usbHostState & (STATE_MASK | SUBSTATE_MASK)) + NEXT_SUBSTATE; }
#define _USB_SetNextSubSubState()                                                                                      \
  { usbHostState = usbHostState + NEXT_SUBSUBSTATE; }
#define _USB_SetNextTransferState()                                                                                    \
  { pCurrentEndpoint->transferState++; }
#define _USB_SetPreviousSubSubState()                                                                                  \
  { usbHostState = usbHostState - NEXT_SUBSUBSTATE; }
#define _USB_SetTransferErrorState(x)                                                                                  \
  { x->transferState = (x->transferState & TSTATE_MASK) | TSUBSTATE_ERROR; }
