...
//************************************************************
static uint8_t
AndroidCommandSendString(void* handle, ANDROID_ACCESSORY_STRINGS stringType, const char* string, uint16_t stringLength);
...
//************************************************************
#define ReadWORD(dest, source)                                                                                         \
  { memcpy(dest, source, 2); }
#define ReadDWORD(dest, source)                                                                                        \
...
/****************************************************************************
  ...
  ***************************************************************************/
void
AndroidAppStart(ANDROID_ACCESSORY_INFORMATION* info) {
  accessoryInfo = info;
...
/****************************************************************************
  ...
  ***************************************************************************/
uint8_t
AndroidAppWrite(void* handle, uint8_t* data, uint32_t size) {
  uint8_t errorCode;
  ...
  errorCode = USBHostWrite(device->address, ANDROID_GetOUTEndpointNum(device), data, size);
...
/****************************************************************************
  ...
  ***************************************************************************/
bool
AndroidAppIsWriteComplete(void* handle, uint8_t* errorCode, uint32_t* size) {
  ANDROID_DEVICE_DATA* device = (ANDROID_DEVICE_DATA*)handle;
  ...
  if(USBHostTransferIsComplete(device->address, ANDROID_GetOUTEndpointNum(device), errorCode, size) == true) {
...
/****************************************************************************
  ...
  ***************************************************************************/
uint8_t
AndroidAppRead(void* handle, uint8_t* data, uint32_t size) {
  ANDROID_DEVICE_DATA* device = (ANDROID_DEVICE_DATA*)handle;
  ...
  errorCode = USBHostRead(device->address,
                          ANDROID_GetINEndpointNum(device),
                          data,
                          ((size / device->INEndpointSize) * device->INEndpointSize));
...
/****************************************************************************
  ...
  ***************************************************************************/
bool
AndroidAppIsReadComplete(void* handle, uint8_t* errorCode, uint32_t* size) {
  ANDROID_DEVICE_DATA* device = (ANDROID_DEVICE_DATA*)handle;
  ...
  // If there was a transfer pending, then get the state of the transfer
  if(USBHostTransferIsComplete(device->address, ANDROID_GetINEndpointNum(device), errorCode, size) == true) {
  ...
  for(i = 0; i < NUM_ANDROID_DEVICES_SUPPORTED; i++) {
    ...
    switch(device->state) {
      ...
      case SEND_MANUFACTUER_STRING:
        ...
        if(AndroidIsLastCommandComplete(device->address, &errorCode, &uint8_tCount) == true) {
          // If not, then let's send the manufacturer's string
          AndroidCommandSendString(device,
                                   ANDROID_ACCESSORY_STRING_MANUFACTURER,
                                   accessoryInfo->manufacturer,
                                   accessoryInfo->manufacturer_size);
      ...
      case SEND_MODEL_STRING:
        ...
        if(device->status.EP0TransferPending == 0) {
          ...
          if(AndroidIsLastCommandComplete(device->address, &errorCode, &uint8_tCount) == true) {
            // If not, then let's send the manufacturer's string
            AndroidCommandSendString(device,
                                     ANDROID_ACCESSORY_STRING_MODEL,
                                     accessoryInfo->model,
                                     accessoryInfo->model_size);
      ...
      case SEND_DESCRIPTION_STRING:
        ...
        if(device->status.EP0TransferPending == 0) {
          ...
          if(AndroidIsLastCommandComplete(device->address, &errorCode, &uint8_tCount) == true) {
            // If not, then let's send the manufacturer's string
            AndroidCommandSendString(device,
                                     ANDROID_ACCESSORY_STRING_DESCRIPTION,
                                     accessoryInfo->description,
                                     accessoryInfo->description_size);
      ...
      case SEND_VERSION_STRING:
        ...
        if(device->status.EP0TransferPending == 0) {
          ...
          if(AndroidIsLastCommandComplete(device->address, &errorCode, &uint8_tCount) == true) {
            // If not, then let's send the manufacturer's string
            AndroidCommandSendString(device,
                                     ANDROID_ACCESSORY_STRING_VERSION,
                                     accessoryInfo->version,
                                     accessoryInfo->version_size);
      ...
      case SEND_SERIAL_STRING:
        ...
        if(device->status.EP0TransferPending == 0) {
          ...
          if(AndroidIsLastCommandComplete(device->address, &errorCode, &uint8_tCount) == true) {
            // If not, then let's send the manufacturer's string
            AndroidCommandSendString(device,
                                     ANDROID_ACCESSORY_STRING_SERIAL,
                                     accessoryInfo->serial,
                                     accessoryInfo->serial_size);
      ...
      case SEND_AUDIO_MODE:
        if((device->protocol < 2) || (accessoryInfo->audio_mode == ANDROID_AUDIO_MODE__NONE)) {
      ...
      case HID_REPORT_DESCRIPTORS_COMPLETE:
        if(device->status.EP0TransferPending == 0) {
          USB_HOST_APP_EVENT_HANDLER(devices[i].address,
                                     EVENT_ANDROID_HID_REGISTRATION_COMPLETE,
                                     &devices[i],
                                     sizeof(ANDROID_DEVICE_DATA*));
          device->state = READY;
        }
        break;
...
/****************************************************************************
  ...
  ***************************************************************************/
bool
AndroidAppInitialize(uint8_t address, uint32_t flags, uint8_t clientDriverID) {
  ...
  for(i = 0; i < NUM_ANDROID_DEVICES_SUPPORTED; i++) {
    if((devices[i].state == WAITING_FOR_ACCESSORY_RETURN) ||
       ((flags & ANDROID_INIT_FLAG_BYPASS_PROTOCOL) == ANDROID_INIT_FLAG_BYPASS_PROTOCOL)) {
      device = &devices[i];
  ...
  while(config_descriptor < config_desc_end) {
    ...
    if((config_descriptor[USB_INTERFACE_DESC_BINTERFACECLASS_OFFSET] == 0xFF) &&
      ...
      while((config_descriptor[USB_DESC_BDESCRIPTORTYPE_OFFSET] != USB_DESCRIPTOR_INTERFACE) &&
            (config_descriptor < config_desc_end)) {
        if(config_descriptor[USB_DESC_BDESCRIPTORTYPE_OFFSET] == USB_DESCRIPTOR_ENDPOINT) {
          ...
          if((config_descriptor[USB_ENDPOINT_DESC_BENDPOINTADDRESS_OFFSET] & 0x80) == 0x80) {
            ...
            device->INEndpointSize = (config_descriptor[USB_ENDPOINT_DESC_WMAXPACKETSIZE_OFFSET]) +
                                     (config_descriptor[USB_ENDPOINT_DESC_WMAXPACKETSIZE_OFFSET + 1] << 8);
          } else {
            ...
            // record the endpoint size (2 uint8_ts)
            device->OUTEndpointSize = (config_descriptor[USB_ENDPOINT_DESC_WMAXPACKETSIZE_OFFSET]) +
                                      (config_descriptor[USB_ENDPOINT_DESC_WMAXPACKETSIZE_OFFSET + 1] << 8);
          }
        }
        config_descriptor += config_descriptor[USB_DESC_BLENGTH_OFFSET];
...
/****************************************************************************
  ...
  ***************************************************************************/
bool
AndroidAppDataEventHandler(uint8_t address, USB_EVENT event, void* data, uint32_t size) {
  uint8_t i;
...
/****************************************************************************
  ...
  ***************************************************************************/
bool
AndroidAppEventHandler(uint8_t address, USB_EVENT event, void* data, uint32_t size) {
    ...
    case EVENT_TRANSFER: // A USB transfer has completed - NOT USED
      ...
      if(transfer_data->bEndpointAddress == 0x00) {
        ...
        if(device->hid.HIDEventSent == 1) {
          ...
          USB_HOST_APP_EVENT_HANDLER(device->address,
...
uint8_t
AndroidAppHIDSendEvent(uint8_t address, uint8_t id, uint8_t* report, uint8_t length) {
  ANDROID_DEVICE_DATA* device = NULL;
...
bool
AndroidAppHIDRegister(uint8_t address, uint8_t id, uint8_t* descriptor, uint8_t length) {
  ANDROID_DEVICE_DATA* device = NULL;
...
/****************************************************************************
  Function:
    static uint8_t AndroidCommandSendString(void *handle, ANDROID_ACCESSORY_STRINGS stringType, const char *string,
  ...
  ***************************************************************************/
static uint8_t
AndroidCommandSendString(void* handle,
                         ANDROID_ACCESSORY_STRINGS stringType,
                         const char* string,
                         uint16_t stringLength) {
  ANDROID_DEVICE_DATA* device = (ANDROID_DEVICE_DATA*)handle;
...
/****************************************************************************
  ...
  ***************************************************************************/
static uint8_t
AndroidCommandStart(void* handle) {
  ANDROID_DEVICE_DATA* device = (ANDROID_DEVICE_DATA*)handle;
...
/****************************************************************************
  ...
  ***************************************************************************/
static bool
AndroidIsLastCommandComplete(uint8_t address, uint8_t* errorCode, uint32_t* uint8_tCount) {
...
/****************************************************************************
  ...
  ***************************************************************************/
static uint8_t
AndroidCommandGetProtocol(ANDROID_DEVICE_DATA* device, uint16_t* protocol) {
  return USBHostIssueDeviceRequest(device->address,         // uint8_t deviceAddress,
