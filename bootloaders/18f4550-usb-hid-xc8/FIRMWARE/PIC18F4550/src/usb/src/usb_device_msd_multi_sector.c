...
#define LUNSectorWrite(bLBA, pDest, Write0)                                                                            \
  LUN[LUN_INDEX].SectorWrite(LUN[LUN_INDEX].mediaParameters, bLBA, pDest, Write0)
  ...
  switch(MSD_State) {
    case MSD_WAIT: // idle state, when we are waiting for a command from the host
    {
      ...
      if(!USBHandleBusy(USBMSDOutHandle)) {
        ...
        } // end of: if((USBHandleGetLength(USBMSDOutHandle) == MSD_CBW_SIZE) && (gblCBW.dCBWSignature ==
          // MSD_VALID_CBW_SIGNATURE))
...
MSDProcessCommandMediaPresent(void) {
  ...
  switch(MSDCommandState) {
    ...
    case MSD_INQUIRY: {
      ...
      memcpy((void*)&msd_buffer[0],
             (const void*)&inq_resp,
             sizeof(InquiryResponse)); // Inquiry response is 36 bytes total
...
MSDReadHandler(void) {
  ...
  switch(MSDReadState) {
    case MSD_READ10_WAIT:
      ...
      if(MSDCheckForErrorCases(TransferLength.Val * (uint32_t)FILEIO_CONFIG_MEDIA_SECTOR_SIZE) !=
         MSD_ERROR_CASE_NO_ERROR) {
        break;
      }
    ...
    case MSD_READ10_XMITING_DATA:
      ...
      if(NewDataAlreadyAvailable == false) {
        ...
        if(fetchStatus == FILEIO_SD_ASYNC_READ_NEW_PACKET_READY) // Check if the MDD_SDSPI_AsyncReadTasks() function is
                                                                 // about to return on data on the next call
        {
          fetchStatus = LUNAsyncReadTasks(&AsyncReadWriteInfo);
      ...
      if((!USBHandleBusy(USBMSDInHandle)) && (NewDataAlreadyAvailable == true)) {
        ...
        AsyncReadWriteInfo.pBuffer =
            (uint8_t*)ptrNextData; // Swap pointer, so we use the other buffer (the one not currently in use by USB).
...
MSDWriteHandler(void) {
  ...
  switch(MSDWriteState) {
    ...
    case MSD_WRITE10_RX_SECTOR: {
      ...
      // data.
      if((PacketsRemainingToBeReceived != 0) && (!USBHandleBusy(NextHandleToCheck)) &&
         (BufferredBytes < (2u * MSD_OUT_EP_SIZE))) {
