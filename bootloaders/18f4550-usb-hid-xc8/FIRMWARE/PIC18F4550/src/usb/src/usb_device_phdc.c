...
#elif defined(__18F2455) || defined(__18F2550) || defined(__18F4455) || defined(__18F4550) || defined(__18F2458) ||    \
    defined(__18F2553) || defined(__18F4458) || defined(__18F4553) || defined(__18LF24K50) || defined(__18F24K50) ||   \
    defined(__18LF25K50) || defined(__18F25K50) || defined(__18LF45K50) || defined(__18F45K50)
...
/**************************************************************************
  ...
  **************************************************************************/
void
USBDevicePHDCInit(USB_PHDC_CB callback) {
...
USBDevicePHDCReceiveData(UINT8 qos, UINT8* buffer, UINT16 len) {
  ...
  recv_endpoint->PHDCDataOutHandle = USBRxOnePacket(recv_endpoint->ep_num, (UINT8*)phdc_data_rx, recv_endpoint->size);
...
USBDevicePHDCSendData(UINT8 qos, UINT8* data, UINT16 length, BOOL memtype) {
  ...
  tx_endpoint->PHDCDataInHandle = USBTxOnePacket(tx_endpoint->ep_num, (BYTE*)phdc_data_tx, tx_endpoint->bytes_to_send);
  USBDevicePHDCUpdateStatus(tx_endpoint->ep_num,
                            1); // update the endpoint status. '1' if endpoint has data. '0' otherwise.
  USBUnmaskInterrupts();
...
USBDevicePHDCTxRXService(USTAT_FIELDS* pdata) {
  ...
  if(USBHALGetLastDirection(val) == DIR_IN) {
    ...
    tx_endpoint->PHDCDataInHandle =
        USBTxOnePacket(tx_endpoint->ep_num, (UINT8*)phdc_data_tx, tx_endpoint->bytes_to_send);
    USBDevicePHDCUpdateStatus(tx_endpoint->ep_num, 1);
    USBUnmaskInterrupts();
  }
  ...
  else // DIR_OUT
  {
    ...
    if((recv_endpoint->offset >= recv_endpoint->transfer_size) || ((recv_endpoint->len % recv_endpoint->size != 0))) {
      if(((recv_endpoint->transfer_size % recv_endpoint->size) == 0) && (recv_endpoint->len != 0)) {
    ...
    USBRxOnePacket(recv_endpoint->ep_num, (BYTE*)phdc_data_rx, (recv_endpoint->transfer_size - recv_endpoint->offset));
  }
  USBUnmaskInterrupts();
}
