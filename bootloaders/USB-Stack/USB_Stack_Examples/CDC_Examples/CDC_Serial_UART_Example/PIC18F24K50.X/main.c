 ...
 * @date 20/02/2019
 *
 * CDC Serial UART Example (for PIC18F24K50/PIC18FX5K50).
...
main(void) {
  ...
  while(1) {
    ...
    } // Pause if not configured or suspended.
    VCP_Tasks();
  }
...
void
CDC_SetControlLineState(void) {
#ifdef USE_RTS
  if(SetControlLineStateData.RTS_bit)
    USB_HAS_SET_RTS = true;
  else
    USB_HAS_SET_RTS = false;

  // If the buffer is not full and the computer wants to set RTS, set RTS.
  if((RxBufferFull == false) && USB_HAS_SET_RTS)
    RTS = RTS_ACTIVE;
  else
    RTS = RTS_ACTIVE ^ 1;
#endif
#ifdef USE_DTR
  if(SetControlLineStateData.DTR_bit)
    DTR = DTR_ACTIVE;
  else
    DTR = DTR_ACTIVE ^ 1;
#endif
...
VCP_Tasks(void) {
  ...
  if(PIR1bits.RC1IF) {
    ...
    if(RxIndex < RX_BUFFER_SIZE)
      RxBuffer[RxIndex++] = UART_Read(); // If RxIndex is not out of bounds, read the Rx data and put in buffer.
