 ...
 */
void
CDC_Arm_COM_EP_IN(void) {
  if(CDC_COM_EP_IN_DATA_TOGGLE_VAL)
    BDT.Array[CDC_COM_BD_IN].STAT = _DTSEN | _DTS; // DATA1
  else
    BDT.Array[CDC_COM_BD_IN].STAT = _DTSEN; // DATA0
 ...
 */
void
CDC_Arm_DAT_EP_OUT(void) {
  if(CDC_DAT_EP_OUT_DATA_TOGGLE_VAL)
    BDT.Array[CDC_DAT_BD_OUT].STAT = _DTSEN | _DTS; // DATA1
  else
    BDT.Array[CDC_DAT_BD_OUT].STAT = _DTSEN; // DATA0
 ...
 */
void
CDC_Arm_DAT_EP_IN(uint16_t cnt) {
  if(CDC_DAT_EP_IN_DATA_TOGGLE_VAL)
    BDT.Array[CDC_DAT_BD_IN].STAT = _DTSEN | _DTS; // DATA1
  else
    BDT.Array[CDC_DAT_BD_IN].STAT = _DTSEN; // DATA0
  BDT.Array[CDC_DAT_BD_IN].CNT = cnt;
  BDT.Array[CDC_DAT_BD_IN].STAT |= _UOWN;
}

bool
CDC_ClassRequest(void) {
  switch(SetupData.bRequest) {
    ...
    case GET_LINE_CODING:
      ...
      if(bytes_available < SetGetLineCodingData.Size_of_Structure) {
        ...
        if(SetGetLineCodingData.Size_of_Structure % bytes_available)
          send_short = true;
        else
          send_short = false;
      } else {
    ...
    case GET_ENCAPSULATED_RESPONSE:
      ...
      if(bytes_available < SetupData.wLength) {
        bytes_2_send = bytes_available;
        if(SetupData.wLength % bytes_available)
          send_short = true;
        else
          send_short = false;
      } else {
