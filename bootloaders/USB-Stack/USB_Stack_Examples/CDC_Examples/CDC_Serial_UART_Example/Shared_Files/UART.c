...
void
UART1_Read_Text(uint8_t* Output, uint8_t* Delimiter, uint8_t Attempts) {
  uint8_t i;
  uint8_t x = 0;
  ...
  while(1) { // Clears all elements in Output array
    if(*Output == 0) {
      break;
    } // until it sees the first Null character
    else {
      *Output = 0;
      Output++;
    }
  ...
  for(i = x; i != 0; i--, Output--) {
    *Output = 0;
  } // Remove delimiter
}
void
UART1_Write(uint8_t data_) {
  while(!UART1_Tx_Idle()) {
  };
  U1TXREG = data_;
}
void
UART1_Write_Text(uint8_t* UART_text) {
  while(*UART_text) {
...
void
UART1_Write_Const(const uint8_t data_) {
  while(!UART1_Tx_Idle()) {
  };
  TXREG = data_;
}
...
void
UART1_Write_Const_Text(const uint8_t* UART_text) {
  while(*UART_text) {
...
void
UART2_Read_Text(uint8_t* Output, uint8_t* Delimiter, uint8_t Attempts) {
  uint8_t i;
  uint8_t x = 0;
  uint16_t del_address = (uint16_t)Delimiter;
  uint16_t out_address = (uint16_t)Output;
...
void
UART2_Write(uint8_t data_) {
  while(!UART2_Tx_Idle()) {
  };
  TXREG2 = data_;
}
void
UART2_Write_Const(const uint8_t data_) {
  while(!UART2_Tx_Idle()) {
  };
  TXREG2 = data_;
}
void
UART2_Write_Text(uint8_t* UART_text) {
  while(*UART_text) {
    UART2_Write(*UART_text++);
  }
}
void
UART2_Write_Const_Text(const uint8_t* UART_text) {
  while(*UART_text) {
