...
// HID Requests
enum { GET_REPORT = 1, GET_IDLE, GET_PROTOCOL, SET_REPORT = 9, SET_IDLE, SET_PROTOCOL };
 ...
 */
void
HID_Arm_EP_OUT(uint8_t BDT_Index) {
  if(EPn_Stat.Array[HID_EP][OUT].Data_Toggle_Val)
 ...
 */
void
HID_Arm_EP_IN(uint8_t BDT_Index, uint16_t cnt) {
  if(EPn_Stat.Array[HID_EP][IN].Data_Toggle_Val)
    BDT.Array[BDT_Index].STAT = _DTSEN | _DTS; // DATA1
  else
    BDT.Array[BDT_Index].STAT = _DTSEN; // DATA0
...
HID_Arm_EP_OUT(void) {
  if(EPn_Stat.Array[HID_EP][OUT].Data_Toggle_Val)
    BDT.Array[HID_BD_OUT].STAT = _DTSEN | _DTS; // DATA1
  else
    BDT.Array[HID_BD_OUT].STAT = _DTSEN; // DATA0
 ...
 */
void
HID_Arm_EP_IN(uint16_t cnt) {
  if(EPn_Stat.Array[HID_EP][IN].Data_Toggle_Val)
    BDT.Array[HID_BD_IN].STAT = _DTSEN | _DTS; // DATA1
  else
    BDT.Array[HID_BD_IN].STAT = _DTSEN; // DATA0
 ...
 */
void
HID_ClearHalt(uint8_t BDT_Index, uint8_t EP, uint8_t dir) {
 ...
 */
void
HID_SendReport(uint8_t ReportNum) {
  if(ReportSent) {
#if PINGPONG_MODE == PINGPONG_1_15 || PINGPONG_MODE == PINGPONG_ALL_EP
    if(HID_EP_IN_LAST_PPB == ODD) {
      USB_RAMCopy((uint8_t*)In_Report_Ptr[ReportNum],
                  (uint8_t*)HID_EP_IN_EVEN_BUFFER_BASE_ADDR,
                  In_Report_Size[ReportNum]);
      HID_Arm_EP_IN(HID_BD_IN_EVEN, In_Report_Size[ReportNum]);
    } else {
      USB_RAMCopy((uint8_t*)In_Report_Ptr[ReportNum],
                  (uint8_t*)HID_EP_IN_ODD_BUFFER_BASE_ADDR,
                  In_Report_Size[ReportNum]);
...
HID_ServiceSOF(void) {
  if(USB_STATE == Configured) {
    ...
    uint8_t i;
    for(i = 0; i < HID_NUM_IN_REPORTS; i++) {
      if(In_Report_Settings[i].Idle_Duration_1ms != 0)
        In_Report_Settings[i]
            .Idle_Count++; // If the Idle_Duration for a report is not infinite (0), increment their counter.
    }

    for(i = 0; i < HID_NUM_IN_REPORTS;
        i++) { // If an Idle_Counter is not infinite and goes past the Idle_Duration, send the report.
      if(In_Report_Settings[i].Idle_Duration_1ms != 0 &&
         In_Report_Settings[i].Idle_Count > In_Report_Settings[i].Idle_Duration_1ms - 1)
        In_Report_Settings[i].Idle_Count_Overflow = true;
    }
#else
    if(In_Report_Settings[0].Idle_Duration_1ms != 0) {
      ...
      if(In_Report_Settings[0].Idle_Count > In_Report_Settings[0].Idle_Duration_1ms - 1)
        In_Report_Settings[0].Idle_Count_Overflow = true;
    }
#endif
  }
}
