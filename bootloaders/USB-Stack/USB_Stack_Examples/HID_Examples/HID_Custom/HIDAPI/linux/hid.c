...
static void
register_error(hid_device* device, const char* op) {}

/* Get an attribute value from a udev_device and return it as a whar_t
   string. The returned string must be freed with free() when done.*/
static wchar_t*
copy_udev_string(struct udev_device* dev, const char* udev_name) {
  const char* str;
  wchar_t* ret = NULL;
...
static int
uses_numbered_reports(__u8* report_descriptor, __u32 size) {
  int i = 0;
  int size_code;
...
static int
get_device_string(hid_device* dev, const char* key, wchar_t* string, size_t maxlen) {
  struct udev* udev;
...
hid_enumerate(unsigned short vendor_id, unsigned short product_id) {
  ...
  udev_list_entry_foreach(dev_list_entry, devices) {
    ...
    /* The device pointed to by hid_dev contains information about
       ...
       it.*/
    dev = udev_device_get_parent_with_subsystem_devtype(hid_dev, "usb", "usb_device");
    if(!dev) {
