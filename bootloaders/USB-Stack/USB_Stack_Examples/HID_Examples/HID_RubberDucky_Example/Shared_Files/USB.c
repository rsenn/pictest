 ...
 *
 * The <i>USB_Tasks()</i> function is designed to run frequently in the main program loop to handle all USB tasks when
 * polling method is used and run in ISR when interrupts are enabled. The function contains the USB Device state machine
 * as seen in <i>Section 9.1.1</i> of the <i>USB Specification 2.0</i>.
 */
void
USB_Tasks(void) {
    ...
    if(TRANSACTION_EP == EP0) {
      if(TRANSACTION_DIR == OUT) {
        ...
        if(ControlStage == DATA_IN_STAGE) {
          ...
          if(UpdateAddress) {
            UADDR = saved_address;
            if(USB_STATE == Default && saved_address != 0)
              USB_STATE = Address;
            else if(saved_address == 0)
              ResetConditionFlag = 1;
            UpdateAddress = false;
          }
        }
      }
 ...
 */
void
USB_ArmEndpoint(BD* BD_ptr, EPS_Type* EPS_ptr, uint16_t buffer_addr, uint16_t cnt) {
 ...
 */
void
USB_Arm_EP0_IN(uint8_t BDT_Index, uint16_t cnt) {
  if(EPn_Stat.Array[EP0][IN].Data_Toggle_Val)
    BDT.Array[BDT_Index].STAT = _DTSEN | _DTS; // DATA1
  else
    BDT.Array[BDT_Index].STAT = _DTSEN; // DATA0
 ...
 */
void
USB_Arm_EP0_IN(uint16_t cnt) {
  if(EPn_Stat.Array[EP0][IN].Data_Toggle_Val)
    BDT.Array[BD0_IN].STAT = _DTSEN | _DTS; // DATA1
  else
    BDT.Array[BD0_IN].STAT = _DTSEN; // DATA0
 ...
 */
void
USB_ArmINStatus(void) {
#if PINGPONG_MODE == PINGPONG_ALL_EP
  if(EP0_IN_LAST_PPB == ODD)
    USB_ArmStatus(&BDT.Array[BD0_IN_EVEN]);
  else
    USB_ArmStatus(&BDT.Array[BD0_IN_ODD]);
#else
  USB_ArmStatus(&BDT.Array[BD0_IN]);
#endif
}
...
#if PINGPONG_MODE == PINGPONG_0_OUT || PINGPONG_MODE == PINGPONG_ALL_EP
  if(PINGPONG_PARITY == ODD)
    USB_RAMCopy(EP0_OUT_ODD, SetupData.Array, 8); // Store Setup Packet Data and straight-away re-arm EP0 OUT.
  else
    USB_RAMCopy(EP0_OUT_EVEN, SetupData.Array, 8);
#else
...
USB_InControlTransfer(void) {
  ...
  if(bytes_2_send) {
    if(bytes_2_send > EP0_SIZE) {
      ...
      if(SendingFrom == ROM)
        USB_ROMCopy((const uint8_t*)((uint24_t)ROMptr), EPaddress, bytes_2_send);
      else
        USB_RAMCopy((uint8_t*)((uint24_t)RAMptr), EPaddress, bytes_2_send);
  ...
  if(bytes_2_send) {
    if(bytes_2_send > EP0_SIZE) {
      ...
      if(SendingFrom == ROM)
        USB_ROMCopy((const uint8_t*)((uint24_t)ROMptr), EP0_IN, bytes_2_send);
      else
        USB_RAMCopy((uint8_t*)((uint24_t)RAMptr), EP0_IN, bytes_2_send);
 ...
 */
void
USB_OutControlTransfer(void) {
#if PINGPONG_MODE == PINGPONG_0_OUT || PINGPONG_MODE == PINGPONG_ALL_EP
  if(bytes_2_recv > EP0_SIZE) {
    if(PINGPONG_PARITY == EVEN)
      USB_RAMCopy(EP0_OUT_EVEN, RAMptr, EP0_SIZE);
    else
      USB_RAMCopy(EP0_OUT_ODD, RAMptr, EP0_SIZE);
    RAMptr += EP0_SIZE;
    bytes_2_recv -= EP0_SIZE;
  } else {
    if(PINGPONG_PARITY == EVEN)
      USB_RAMCopy(EP0_OUT_EVEN, RAMptr, bytes_2_recv);
    else
      USB_RAMCopy(EP0_OUT_ODD, RAMptr, bytes_2_recv);
    RAMptr += EP0_SIZE;
    bytes_2_recv = 0;
  }
#else
 ...
 */
void
USB_ROMCopy(const uint8_t* ROMaddress, uint8_t* EPaddress, uint8_t bytes) {
  uint8_t i;
  for(i = 0; i < bytes; i++) {
 ...
 */
void
USB_RAMCopy(uint8_t* RAM1address, uint8_t* RAM2address, uint8_t bytes) {
  uint8_t i;
  for(i = 0; i < bytes; i++) {
 ...
 */
void
USB_RAMSet(uint8_t val, uint8_t* RAMaddress, uint16_t bytes) {
  uint16_t i;
    ...
    switch(GetStatusData.bmRequestType_bits.Recipient) {
      ...
      case Endpoint:
        if(USB_STATE == Address) {
          ...
          if(GetStatusData.ZeroInterfaceEndpoint_bits.Endpoint_bits.EndpointNumber < NUM_ENDPOINTS)
            Perform_Request_Error = false;
        }
        if(Perform_Request_Error == false) {
          GetStatusReturnedData.Halt = EPn_Stat
                                           .Array[GetStatusData.ZeroInterfaceEndpoint_bits.Endpoint_bits.EndpointNumber]
                                                 [GetStatusData.ZeroInterfaceEndpoint_bits.Endpoint_bits.Direction]
                                           .Halt;
        }
        break;
    }
      ...
      case Device:
        if(SetClearFeatureData.FeatureSelector == DEVICE_REMOTE_WAKEUP) {
          if(SetupData.bRequest == CLEAR_FEATURE)
            DEVS.Remote_Wakeup = REMOTE_WAKEUP_OFF;
          else
            DEVS.Remote_Wakeup = REMOTE_WAKEUP_ON;
          Perform_Request_Error = false;
        }
        break;
      case Endpoint:
        if(SetClearFeatureData.FeatureSelector == ENDPOINT_HALT) {
          if(USB_STATE == Configured) {
            if(SetClearFeatureData.ZeroInterfaceEndpoint_bits.Endpoint_bits.EndpointNumber < NUM_ENDPOINTS)
              Perform_Request_Error = false;
          } else {
...
#if PINGPONG_MODE == PINGPONG_DIS
              BDT_Index = (SetClearFeatureData.ZeroInterfaceEndpoint_bits.Endpoint_bits.EndpointNumber * 2u) +
                          SetClearFeatureData.ZeroInterfaceEndpoint_bits.Endpoint_bits.Direction;
#elif PINGPONG_MODE == PINGPONG_0_OUT
              BDT_Index = (SetClearFeatureData.ZeroInterfaceEndpoint_bits.Endpoint_bits.EndpointNumber * 2u) + 1u +
                          SetClearFeatureData.ZeroInterfaceEndpoint_bits.Endpoint_bits.Direction;
#elif PINGPONG_MODE == PINGPONG_1_15
              BDT_Index = (SetClearFeatureData.ZeroInterfaceEndpoint_bits.Endpoint_bits.EndpointNumber * 2u) + 1u +
                          SetClearFeatureData.ZeroInterfaceEndpoint_bits.Endpoint_bits.Direction;
#else
              BDT_Index = (SetClearFeatureData.ZeroInterfaceEndpoint_bits.Endpoint_bits.EndpointNumber * 2u) + 2u +
                          (SetClearFeatureData.ZeroInterfaceEndpoint_bits.Endpoint_bits.Direction * 2u);
#endif
              if(SetupData.bRequest == CLEAR_FEATURE) {
                USB_AppClearHalt(BDT_Index,
                                 SetClearFeatureData.ZeroInterfaceEndpoint_bits.Endpoint_bits.EndpointNumber,
                                 SetClearFeatureData.ZeroInterfaceEndpoint_bits.Endpoint_bits.Direction);
              } else {
                EPn_Stat
                    .Array[SetClearFeatureData.ZeroInterfaceEndpoint_bits.Endpoint_bits.EndpointNumber]
                          [SetClearFeatureData.ZeroInterfaceEndpoint_bits.Endpoint_bits.Direction]
                    .Halt = 1;
...
USB_GET_CONFIGURATION(void) {
  ...
  if(USB_STATE == Configured)
    GetConfigurationReturnedData = CurrentConfiguration;
  else
    GetConfigurationReturnedData = 0; // If any state other than configured, return 0
#if PINGPONG_MODE == PINGPONG_ALL_EP
  if(EP0_IN_LAST_PPB == ODD) {
...
USB_SET_CONFIGURATION(void) {
  if((USB_STATE == Address || USB_STATE == Configured) &&
     (SetConfigurationData.ConfigurationValue < NUM_CONFIGURATIONS + 1)) {
