 ...
 * @date 04/03/2019
 *
 * MSD Internal Example (for PIC18F24K50).
...
} BOOT16 @BOOT_START = {{0xEB, 0x3C, 0x90},
                        {'M', 'S', 'D', 'O', 'S', '5', '.', '0'},
                        BYTES_PER_BLOCK_LE,
                        1,
                        1,
...
const struct {
  ...
  uint32_t FileSize;
    ...
    {'U', 'S', 'B', ' ', 'D', 'R', 'I', 'V', 'E', ' ', ' '}, 0x08, 0, 0, 0, 0, 0, 0, 0x819A, 0x4E64, 0, 0};
...
MSD_RxSector(void) {
  ...
  if(addr < END_OF_FLASH) { // If address is in flash space.
    if(MSD_EP_IN_LAST_PPB == ODD)
      Flash_ReadBytes((uint24_t)(addr + ByteOfSect), 64, MSD_EP_IN_ODD); // Read flash into MSD_SectData buffer.
    else
      Flash_ReadBytes((uint24_t)(addr + ByteOfSect), 64, MSD_EP_IN_EVEN);
  } else {
    if(MSD_EP_IN_LAST_PPB == ODD)
      USB_RAMSet(0, MSD_EP_IN_ODD, 64);
    else
      USB_RAMSet(0, MSD_EP_IN_EVEN, 64);
  }
#else
  if(addr < END_OF_FLASH)
    Flash_ReadBytes((uint24_t)(addr + ByteOfSect), 64, MSD_EP_IN); // Read flash into MSD_SectData buffer.
  else
    USB_RAMSet(0, MSD_EP_IN, 64);
#endif
}
void
MSD_TxSector(void) {
  ...
  if(addr < END_OF_FLASH) {              // If address is in flash space.
    ...
    if(MSD_EP_OUT_LAST_PPB == ODD)
      Flash_EraseWriteBlock((uint24_t)(addr + ByteOfSect), MSD_EP_OUT_ODD);
    else
      Flash_EraseWriteBlock((uint24_t)(addr + ByteOfSect), MSD_EP_OUT_EVEN);
#else
    Flash_EraseWriteBlock((uint24_t)(addr + ByteOfSect), MSD_EP_OUT);
#endif
  }
}

uint32_t
LBA2FlashAddr(uint32_t LBA) {
