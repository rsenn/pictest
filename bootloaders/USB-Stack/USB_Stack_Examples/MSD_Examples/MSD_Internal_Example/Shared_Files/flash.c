...
void
Flash_ReadBytes(uint16_t start_addr, uint16_t bytes, uint8_t* flash_array) {
  _EECON1 = 0x80;
  while(bytes) {
...
void
Flash_Erase(uint16_t start_addr, uint16_t end_addr) {
  _EECON1 = 0x84;
...
void
Flash_EraseWriteBlock(uint16_t start_addr, uint8_t* flash_array) {
...
void
Flash_WriteBlock(uint16_t start_addr, uint8_t* flash_array) {
#ifdef _PIC14
  uint8_t i;
...
#elif defined(_PIC18)
void
Flash_ReadBytes(uint24_t start_addr, uint24_t bytes, uint8_t* flash_array) {
...
void
Flash_Erase(uint24_t start_addr, uint24_t end_addr) {
...
void
Flash_EraseWriteBlock(uint24_t start_addr, uint8_t* flash_array) {
#if _FLASH_ERASE_SIZE > _FLASH_WRITE_SIZE
  uint8_t i;
...
void
Flash_WriteBlock(uint24_t start_addr, uint8_t* flash_array) {
  uint8_t i;
