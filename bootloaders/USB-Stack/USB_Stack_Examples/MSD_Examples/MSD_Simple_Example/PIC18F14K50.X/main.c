 ...
 * @date 13/02/2019
 *
 * MSD Simple Example (for PIC18F14K50).
...
#error "Can't fit the MSD buffers in double buffer RAM space for pingpong buffering of EPs > EP0 in this example."
#endif
...
/** Volume Root Entry */
const struct ROOT_DIR_TYPE ROOT = {{{'U', 'S', 'B', ' ', 'D', 'R', 'I', 'V', 'E', ' ', ' '},
                                    0x08,
                                    0,
                                    0,
                                    0x7BA0,
...
MSD_RxSector(void) {
  ...
  if(EPn_Stat.Array[MSD_EP][IN].Last_PPB == ODD)
    RSC_EPaddress = MSD_EP_IN_ODD;
  else
    RSC_EPaddress = MSD_EP_IN_EVEN;
#else
#define RSC_EPaddress MSD_EP_IN
  ...
  if(R_W_10_Vars.LBA == BOOT_SECT_ADDR) { // If PC is reading the Boot Sector.
    if(ByteOfSect == 0)
      USB_ROMCopy((const uint8_t*)(&BOOT16), RSC_EPaddress, sizeof(BOOT16));
