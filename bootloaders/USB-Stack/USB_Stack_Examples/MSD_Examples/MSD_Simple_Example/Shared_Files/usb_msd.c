 ...
 */
void
MSD_Arm_EP_OUT(uint8_t BDT_Index) {
  if(EPn_Stat.Array[MSD_EP][OUT].Data_Toggle_Val)
 ...
 */
void
MSD_Arm_EP_IN(uint8_t BDT_Index, uint16_t cnt) {
  if(EPn_Stat.Array[MSD_EP][IN].Data_Toggle_Val)
    BDT.Array[BDT_Index].STAT = _DTSEN | _DTS; // DATA1
  else
    BDT.Array[BDT_Index].STAT = _DTSEN; // DATA0
...
MSD_Arm_EP_OUT(void) {
  if(EPn_Stat.Array[MSD_EP][OUT].Data_Toggle_Val)
    BDT.Array[MSD_BD_OUT].STAT = _DTSEN | _DTS; // DATA1
  else
    BDT.Array[MSD_BD_OUT].STAT = _DTSEN; // DATA0
 ...
 */
void
MSD_Arm_EP_IN(uint16_t cnt) {
  if(EPn_Stat.Array[MSD_EP][IN].Data_Toggle_Val)
    BDT.Array[MSD_BD_IN].STAT = _DTSEN | _DTS; // DATA1
  else
    BDT.Array[MSD_BD_IN].STAT = _DTSEN; // DATA0
 ...
 */
bool
MSD_ClassRequest(void) {
  if(SetupData.bRequest == GET_MAX_LUN) {
    USB_RequestError();
 ...
 */
void
MSD_ClearHalt(uint8_t BDT_Index, uint8_t EP, uint8_t dir) {
  if(!WaitForBOMSR) {
...
MSD_ServiceCBW(void) {
  ...
  if(MSD_EP_OUT_LAST_PPB == ODD)
    USB_RAMCopy(MSD_EP_OUT_ODD, CBW_Data.BYTES, 31);
  else
    USB_RAMCopy(MSD_EP_OUT_EVEN, CBW_Data.BYTES, 31);
#else
  ...
  switch(CBW_Data.CBWCB0[0]) {
    ...
    case READ_10:
      ...
      if((R_W_10_Vars.LBA + R_W_10_Vars.TF_LEN) > VOL_CAPACITY_IN_BLOCKS)
        return MSD_FailCommand(devExpect,
                               ...
                               ASCQ_LOGICAL_BLOCK_ADDRESS_OUT_OF_RANGE);
      ...
      if(CaseResult & (CASE_11 | CASE_12 | CASE_13)) {
        if(CaseResult == CASE_13)
          CSW_Data.bCSWStatus = PHASE_ERROR;
        else
          CSW_Data.bCSWStatus = COMMAND_PASSED;
        ByteOfSect = 0;
#if PINGPONG_MODE == PINGPONG_1_15 || PINGPONG_MODE == PINGPONG_ALL_EP
    ...
    case READ_CAPACITY:
      ...
      if((Read_Capacity_10_Cmd.LOGICAL_BLOCK_ADDRESS != 0) && (Read_Capacity_10_Cmd.PMI == 0)) {
        ...
        R_W_10_Vars.START_LBA_BYTES[0] =
            Read_Capacity_10_Cmd.LOGICAL_BLOCK_ADDRESS_BYTES[3]; // Big-endian to little-endian
    ...
    COMMAND_ERROR:
      ...
      if(CBW_Data.bCBWCBLength) {
        if(CBW_Data.Direction)
          return MSD_FailCommand(Di, ILLEGAL_REQUEST, ASC_INVALID_COMMAND_OPCODE, ASCQ_INVALID_COMMAND_OPCODE);
        else
          return MSD_FailCommand(Do, ILLEGAL_REQUEST, ASC_INVALID_COMMAND_OPCODE, ASCQ_INVALID_COMMAND_OPCODE);
      } else
        return MSD_FailCommand(Dn, ILLEGAL_REQUEST, ASC_INVALID_COMMAND_OPCODE, ASCQ_INVALID_COMMAND_OPCODE);
  }
}
 ...
 */
static void
MSD_CalcResidue(uint16_t CaseResult, uint32_t DeviceBytes) {
 ...
 */
static uint16_t
MSD_Check13Cases(uint32_t DeviceBytes, uint8_t devExpect) {
  ...
  switch(devExpect) {
    ...
    case Di:
      if(CBW_Data.dCBWDataTransferLength == 0)
        ...
        if(CBW_Data.dCBWDataTransferLength == DeviceBytes)
          CaseResult = CASE_6;
        else if(CBW_Data.dCBWDataTransferLength > DeviceBytes)
          CaseResult = CASE_5;
        else
          CaseResult = CASE_7;
      } else
 ...
 */
static uint8_t
MSD_FailCommand(uint8_t devExpect, uint8_t SK, uint8_t ASC, uint8_t ASCQ) {
  uint8_t result;
  if(CBW_Data.dCBWDataTransferLength != 0) {
#if PINGPONG_MODE == PINGPONG_1_15 || PINGPONG_MODE == PINGPONG_ALL_EP
    if(devExpect == Di) {
 ...
 */
static uint8_t
MSD_NoDataResponse(uint8_t status) {
 ...
 */
static uint8_t
MSD_SendDataResponse(uint32_t DeviceBytes) {
  ...
  if(CaseResult & (CASE_2 | CASE_5 | CASE_6 | CASE_7)) {
    if(CaseResult & (CASE_5 | CASE_6 | CASE_7)) {
      ...
      return_val = MSD_NO_DATA_STAGE;

    if(CaseResult & (CASE_5 | CASE_6))
      CSW_Data.bCSWStatus = COMMAND_PASSED;
    else
      CSW_Data.bCSWStatus = PHASE_ERROR;
...
MSD_ServiceRead10(void) {
  ...
  MSD_RxSector();
#else
  for(uint16_t i = 0; i < MSD_EP_SIZE; i++)
    EPaddress[i] = MSD_SectData[i + ByteOfSect]; // Load EP size worth of data from the MSD_SectData buffer.
#endif

  ByteOfSect += MSD_EP_SIZE;
  if(ByteOfSect == BYTES_PER_BLOCK_LE) { // More than one sector is required. Last bytes of sector were sent, increment
                                         // the address, and load new sector.
    R_W_10_Vars.LBA++;
#ifndef MSD_LIMITED_RAM
    MSD_RxSector();
#endif
...
#else
  for(uint16_t i = 0; i < MSD_EP_SIZE; i++)
    MSD_EP_IN[i] = MSD_SectData[i + ByteOfSect]; // Load EP size worth of data from the MSD_SectData buffer.
#endif
 ...
 */
static bool
MSD_ServiceWrite10(void) {
#if PINGPONG_MODE == PINGPONG_1_15 || PINGPONG_MODE == PINGPONG_ALL_EP
#ifndef MSD_LIMITED_RAM
  uint8_t* EPaddress;

  if(MSD_EP_OUT_LAST_PPB == ODD)
    EPaddress = MSD_EP_OUT_ODD;
  else
    EPaddress = MSD_EP_OUT_EVEN;
#endif
...
#else
  for(uint16_t i = 0; i < MSD_EP_SIZE; i++)
    MSD_SectData[i + ByteOfSect] = EPaddress[i]; // Load EP size worth of data from EP to MSD_SectData buffer.
#endif
  ByteOfSect += MSD_EP_SIZE;
...
#else
  for(uint16_t i = 0; i < MSD_EP_SIZE; i++)
    MSD_SectData[i + ByteOfSect] = MSD_EP_OUT[i]; // Load EP size worth of data from EP to MSD_SectData buffer.
#endif
  ByteOfSect += MSD_EP_SIZE;
  if(ByteOfSect == BYTES_PER_BLOCK_LE) {
#ifndef MSD_LIMITED_RAM
