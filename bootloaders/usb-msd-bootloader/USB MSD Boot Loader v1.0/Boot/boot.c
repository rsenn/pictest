 ...
 *					- However, we report only a small FAT size so the (formatted) disk size reported as 511KB (see notes below)
 ...
 *					- BOOT: To exit Boot Mode this file has to be deleted (which is only allowed under strict conditions)
 *
 ...
 *					- To ensure we do not have any time-out issues we process file writes in 64 byte chunks (as the packets for each sector arrive).
 *					  However, file reads are done on a 512 byte sector basis.
 *					- Flash memory write are verified & we assume that USB takes care of any possible transfer errors.
 *
 *					** Quirks
 *					- After an update we attempt to 'refresh' the folder display to show the results of the operation. We do this by reporting that the media
 *					  has been removed (in response to a SCSI Test Unit Ready command). Under Windows the refresh is instant. However, when used with operating
 *					  systems (such as OS X & some Linux) the MSD device will be reported as removed by the OS and will then automatically re-attach.
 *					  You may need to re-open the emulated disk folder or do a folder refresh to correctly view the current status.
 *
 *					- Operating systems such as Linux & Windows 2000 may take up to 50 seconds to write data/delete from the MSD device: WAIT & BE-CAREFUL!
 *					  (Note: If you want to reduce this time for your OS, investigate disabling write caching for handling surprise drive removal. It is fairly
 *					  simple to adjust under Linux. However, unles you are doing a lot of updates it is probably better to wait....)
 *					  OS X and Windows XP SP2 (or later) have no delay.
 *					
...
#define FAT_SECTORS						4L								// Each FAT occupies 4 sectors
																		// Sector offset location of root directory (1st sector)
#define ROOT_DIR_OFFSET					((2 * FAT_SECTORS) + FAT1_SECTOR1)		
#define ROOT_DIR_SECTORS				4L 								// Use four sectors (64 entries)
...
#define FILE_HEADER_SIZE				6								// Files that can be updated must have a valid header to ensure correct operation
...
{	
 	0xEB, 0x3C, 0x90,													// No MBR/Partition table
	'M','S','D','O','S','5','.','0',									// ID
	...
	0x40, 0x00,															// Max 64 entries in root directory
	0x88, 0x13,															// 5000 sectors (2.56MB to ensure FAT16)
	0xF8,																// Non-removable media
	0x04, 0x00,															// Only 4 sectors per FAT. Windows will report a formatted disk
																		// size of 511KB (not 2.56MB).
	0x00, 0x00, 0x00, 0x00,												// N/A
	0x00, 0x00, 0x00, 0x00,												// N/A
	0x00, 0x00, 0x00, 0x00,												// N/A (if < 32k sectors)
	0x00,																// Logical drive #
	0x00,																// N/A
	0x29,																// Extended boot signature
	0xAA, 0xBB, 0xCC, 0xDD,												// Volume serial #
	'N','O',' ','N','A','M','E',' ',' ',' ',' ',						// No volume label
	'F','A','T','1','6',' ',' ',' ',									// FAT 16
...
{
																		// Volume name 
	'L','O','A','D','E','R',' ',' ',									// Volume name
	' ',' ',' ',														// No extension
	0x08,																// Attributes (Volume Label)
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,							// N/A  
	0x00, 0x00, 0x00,													// N/A
	0x00, 0x00,															// Last modification time
	0xC1, 0x38,															// Last modifcation date (1/06/2008)
	0x00, 0x00,															// N/A
	0x00, 0x00, 0x00, 0x00,												// N/A

																		// Boot Mode Indicator File 
	'B','O','O','T',' ',' ',' ',' ',									// Short file name
	' ',' ',' ',														// No extension
	0x00,																// Attributes
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,							// N/A 
	0x00, 0x00, 0x00,													// N/A
	0x00, 0x00,															// Last modification time
	0xC1, 0x38,															// Last modifcation date (1/06/2008)
	0x02, 0x00,															// Starts at cluster 2
	0x00, 0x02, 0x00, 0x00,												// File size is 512 bytes (1 cluster)

																		// Config File 
	'C','O','N','F','I','G',' ',' ',									// Short file name
	' ',' ',' ',														// No extension
	0x00,																// Attributes
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,							// N/A 
	0x00, 0x00, 0x00,													// N/A
	0x00, 0x00,															// Last modification time
	0xC1, 0x38,															// Last modifcation date (1/06/2008)
	0x03, 0x00,															// Starts at cluster 3
	0x00, 0x02, 0x00, 0x00,												// File size is 512 bytes (1 cluster)

																		// Firmware File Version
	'F','W','_',' ',' ',' ',' ',' ',									// Short file name
	' ',' ',' ',														// No Extension
...
byte SectorRead(dword sector_addr, byte *buffer)
{
	...
	switch((word)sector_addr)											// Action depends on sector requested (FAT emulation)
	{
		...
		case FAT1_SECTOR1 + FAT_SECTORS:								// File allocation table copy
			...
			buffer[0] = 0xF8; buffer[1] = 0xFF;							// Non-removable media
			buffer[2] = 0xFF; buffer[3] = 0xFF;							// Formatted volume

		
			buffer[4] = 0xFF; buffer[5] = 0xFF;							// Emulate cluster chain for boot mode indicator file
																		// (1 cluster)

			buffer[8] = 0xFF; buffer[9] = 0xFF;							// Emulate cluster chain for firmware version file
																		// (1 cluster)

			buffer[6] = 0xFF; buffer[7] = 0xFF;							// Emulate cluster chain for config file
																		// (1 cluster)
 ...
 *
 *					This function is actually called from the (modified) MSD class handler every 64 bytes (to minimise chance of OS time-out)
 *
 *
 *****************************************************************************/
byte SectorWrite(dword sector_addr, byte *buffer, BOOL Sector_Start)
{							
	...
	{ 
		...
		if( *(buffer + 7) == 1 ) status.ENCRYPTED = TRUE;				// Set flag if this file is encrypted	
	}

	if( sector_addr >= BOOT_SECTOR && sector_addr < ROOT_DIR_OFFSET )	// Ignore & quit if this is a write to the Boot or FAT sectors
	{
	...
	{		
		...
		if( buffer[ROOT_DIR_BOOT_ENTRY_OFFSET] == DIR_ENTRY_DELETED && status.BOOT_MODE && Sector_Start )
		{
			...
			if( status.UPDATE_FAIL || status.UPDATE_COMPLETE )
			{
				...
				status.UPDATE_COMPLETE ^= 1;							// Set flag to attempt to produce a folder display refresh by emulating media removal
																		// this will bring back the boot file to indicate failure. 
		...
		else if( !Sector_Start && status.BOOT_MODE == TRUE )
		{
			...
			if( (buffer[ROOT_DIR_FWVER_ENTRY_OFFSET] == DIR_ENTRY_DELETED) || (buffer[ROOT_DIR_CONFIG_ENTRY_OFFSET] == DIR_ENTRY_DELETED) )
			{	 
				status.UPDATE_COMPLETE = TRUE;							// Set flag to attempt to produce a Windows folder display refresh by emulating media removal
																		// this will bring back the boot file to indicate failure. 
				return ERROR;											// Quit with error (as file is not deleted)
			}
		}
	...
	// We are going to make the assumtion that the host OS is going to write the file sectors in order starting with the first
	// and that only one file is being written at a time!

	
	// Check locks to see if we are currently writing to any file
	// If we are not writing to any file then check the header to determine which file is going to be updated and set appropriate semaphore																			
	...
	// Note: Firmware update files contain code for the full amount of program memory for the device being updated. Whilst this may make the
	// updates a little slower, it means that the size of each firmware upgrade file is known in advance (i.e. we know when it is completed), and as
	// there are no gaps we do not need to do any boundary checking.
	//
	// We need to check that it is a valid upgrade file sector each time because some OS's write out some other stuff such as directory entries
	// mid-file. To check we simply look for the 6-byte header. 
...
void WriteFlash64(word block_address, byte *block_ptr)
{
	...
	if( block_address > (REMAP_RESET_VECTOR - 1) )						// Update program memory as long as code is NOT within the bootloader area 
		program = TRUE;
...
#ifdef ENCRYPT
			if( status.ENCRYPTED )
			{															// Decrypt 16-bytes (as per HEXStream application)
