...
typedef struct _EP
{
	near byte *EP_Ctl;				// Pointer to end point control register
	BDT *pBDT_In;					// Pointer to buffer descriptor table for IN endpoint
	BDT	*pBDT_Out;					// Pointer to buffer descriptor table for OUT endpoint
	byte *pTX_Buffer;				// Pointer to user buffer for tx to host
	byte *pRX_Buffer;				// Pointer to user buffer for rx from host
	byte TX_Len;					// TX buffer byte count
	byte RX_Len;					// RX buffer byte count
