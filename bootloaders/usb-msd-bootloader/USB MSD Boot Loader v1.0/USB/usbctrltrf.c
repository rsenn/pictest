...
void HandleHIDControlOutReport(void);
...
USBPrepareForNextSetupTrf(void) {
  ...
  ********************************************************************/
  if((ctrl_trf_state == CTRL_TRF_RX) && (UCONbits.PKTDIS == 1) && (ep0Bo.Cnt == sizeof(CTRL_TRF_SETUP)) &&
     (ep0Bo.Stat.PID == SETUP_TOKEN) && (ep0Bo.Stat.UOWN == 0)) {
