 ...
 *					HEXStream can (optionally) encrypt the firmware file (obfuscation) in a simple manner which the
 *Bootloader can decrypt The 'encryption' is included as a basic example. The same  start key needs to be entered into
 *HEXStream and boot.h Please consult accompanying documentation and the file boot.c for detailed description This
 *example is for PICDEM FS USB board Bootloader will remain on Boot Mode unless valid user firmware is loaded and User
 *Mode is requested. A two-byte EEPROM signature determines the current mode.
 *
 * History:
 *
 *
