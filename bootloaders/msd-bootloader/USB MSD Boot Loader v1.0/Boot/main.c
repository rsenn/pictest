 ...
 *					HEXStream can (optionally) encrypt the firmware file (obfuscation) in a simple manner which the Bootloader
 *can decrypt The 'encryption' is included as a basic example. The same  start key needs to be entered into HEXStream
 *and boot.h Please consult accompanying documentation and the file boot.c for detailed description This example is for
 *PICDEM FS USB board Bootloader will remain on Boot Mode unless valid user firmware is loaded and User Mode is
 *requested. A two-byte EEPROM signature determines the current mode.
 *
 * History:
 *
 *
...
main(void) {
  ...
  if(SWITCH_2 && *((rom byte*)(FIRMWARE_VERSION + 2)) == FW_VALID &&
  ...
  while(1) // Infinite task loop
  {
    ...
    if(Timer_200mS == 5) {
      mToggle_Status_LEDS() Timer_200mS = 0;
    }
#endif
...
Handle_Timers(void) {
  ...
  if(Timer_Flag_1mS) // 1mS hardware timer rollover?
  {
    ...
    if(Timer_1mS == 200) Timer_200mS++; // Increment 200mS timer
  }
}
