 ...
 * Ray Gordon			22/04/08	Increased timer in USBRemoteWakeup to ensure correct operation in Extended Mode
 * Ray Gordon			22/04/08	Removed all modifiable code in USBSuspend Function
 ********************************************************************/

/** I N C L U D E S **********************************************************/
#include "Config_IO.h" // Required for USBCheckBusStatus()
#include "usb.h"
...
USBSuspend(void) {
  ...
  /* RPG - 22/4/08: Removed everything to ensure correct suspend mode operation */
  /*
      PIR2bits.USBIF = 0;
      INTCONbits.RBIF = 0;
 ...
 *
 *
 * RPG 22/04/08:	As we are running in extended mode with optimisation @ 48MHz I have
 *					increased the delay count value to 11000
 *
 *****************************************************************************/
void
USBRemoteWakeup(void) {
  ...
  {                              // has been enabled by the host.
    ...
    delay_count = 11000U; // 1800U;    // Set RESUME line for 1-13 ms
    do {
      delay_count--;
    } while(delay_count);
...
/* Auxiliary Function */
void
ClearArray(byte* startAdr, byte count) {
  *startAdr;
  while(count) {
    _asm clrf POSTINC0, 0 _endasm count--;
  } // end while
} // end ClearArray
