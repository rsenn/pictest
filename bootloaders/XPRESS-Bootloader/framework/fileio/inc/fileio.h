...
typedef enum {
  ...
  FILEIO_FORMAT_BOOT_SECTOR // Creates a boot sector based on user-specified information and erases any existing
                            // information
} FILEIO_FORMAT_MODE;
...
typedef enum {
  ...
  FILEIO_ATTRIBUTE_SYSTEM =
      0x04, // System attribute.  A file with this attribute is used by the operating system and should not be modified.
  FILEIO_ATTRIBUTE_VOLUME = 0x08, // Volume attribute.  If the first file in the root directory of a volume has this
                                  // attribute, the entry name is the volume name.
  FILEIO_ATTRIBUTE_LONG_NAME =
      0x0F, // A file entry with this attribute mask is used to store part of the file's Long File Name.
...
// Description: The FILEIO_OBJECT structure is used to hold file information for an open file as it's being modified or
// accessed.  A pointer to
//              an open file's FILEIO_OBJECT structure will be passed to any library function that will modify that
//              file.
typedef struct {
...
/***************************************************************************
    ...
    Returns:
        ...
        If Failure: false
***************************************************************************/
typedef uint8_t (*FILEIO_DRIVER_SectorWrite)(void* mediaConfig,
                                             uint32_t sector_addr,
                                             uint8_t* buffer,
                                             bool allowWriteToZero);
...
***************************************************************************/
int FILEIO_Format(
    FILEIO_DRIVE_CONFIG* config, void* mediaParameters, FILEIO_FORMAT_MODE mode, uint32_t serialNumber, char* volumeId);
