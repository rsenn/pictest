...
#define DRV_FILEIO_INTERNAL_FLASH_NUM_ROOT_DIRECTORY_SECTORS                                                           \
  ((DRV_FILEIO_CONFIG_INTERNAL_FLASH_MAX_NUM_FILES_IN_ROOT + 15) / 16) //+15 because the compiler truncates
#define DRV_FILEIO_INTERNAL_FLASH_OVERHEAD_SECTORS                                                                     \
  (DRV_FILEIO_INTERNAL_FLASH_NUM_RESERVED_SECTORS + DRV_FILEIO_INTERNAL_FLASH_NUM_VBR_SECTORS +                        \
   DRV_FILEIO_INTERNAL_FLASH_NUM_ROOT_DIRECTORY_SECTORS + DRV_FILEIO_INTERNAL_FLASH_NUM_FAT_SECTORS)
#define DRV_FILEIO_INTERNAL_FLASH_TOTAL_DISK_SIZE                                                                      \
  (DRV_FILEIO_INTERNAL_FLASH_OVERHEAD_SECTORS + DRV_FILEIO_INTERNAL_FLASH_CONFIG_DRIVE_CAPACITY)
#define DRV_FILEIO_INTERNAL_FLASH_PARTITION_SIZE                                                                       \
  (uint32_t)(DRV_FILEIO_INTERNAL_FLASH_TOTAL_DISK_SIZE - 1) //-1 is to exclude the sector used for the MBR
...
#warning                                                                                                               \
    "MSD volume overlaps flash erase page with firmware program memory.  Please change your configuration settings to ensure the MSD volume cannot share an erase page with the firmware."
// See code comments in FSconfig.h, and adjust the MDD_INTERNAL_FLASH_DRIVE_CAPACITY definition until the warning goes
// away.
#endif
#endif
...
#error                                                                                                                 \
    "The current implementation of internal flash MDD only supports a media sector size of 512.  Please modify your selected value in the FSconfig.h file."
#endif
