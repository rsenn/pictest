...
// Macro defining the maximum supported sector size for the FILEIO module.  This value should always be 512 , 1024,
// 2048, or 4096 bytes. Most media uses 512-byte sector sizes.
...
// use a single instance of the FAT and Data buffer.  Otherwise, it will use one FAT buffer and one data buffer per
// drive (defined by FILEIO_CONFIG_MAX_DRIVES).  If you are only using one drive in your application, this option has no
// effect.
...
// by carefully choosing the DRV_FILEIO_INTERNAL_FLASH_CONFIG_FILES_ADDRESS and
// DRV_FILEIO_INTERNAL_FLASH_CONFIG_DRIVE_CAPACITY, to make sure the MSD volume does extend into the erase page with the
// configuration bits.
