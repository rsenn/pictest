...
void
APP_writeByte(uint32_t address, uint16_t data) {
  if(!APP_inProgress()) {
...
void
APP_write(uint32_t address, uint8_t* buffer, uint8_t count) {
...
bool
APP_addressInRange(uint32_t address) {
...
void
APP_rowWrite(uint32_t address, uint8_t* buffer, uint8_t count) {
  if(!APP_addressInRange(address)) {
    return;
  }
