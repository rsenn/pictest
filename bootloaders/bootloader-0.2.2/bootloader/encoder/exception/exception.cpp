...
DEBadValue::DEBadValue(string argument, string rangeBegin, string rangeEnd) {
  _message = string("Bad value provided for \"") + argument + string("\" argument.");
  _message += string("\nValue must be in range from ") + rangeBegin + string(" to ") + rangeEnd + (".");
}
...
DEBadValue::DEBadValue(string argument, list<string> values) {
  ...
  for(list<string>::iterator i = values.begin(); i != values.end(); i++) _message += string("\n\t") + (*i);
}
...
DEDevComandNotSupported::DEDevComandNotSupported(string command, string devSubtype, string memory) {
  if(memory == "") {
    _message = string("Command \"") + command + string("\" is not supported by ") + devSubtype + string(" device.");
  } else {
    _message = string("Command \"") + command + string("\" is not supported by ") + devSubtype +
               string(" device for \"") + memory + string("\" memory type.");
  }
...
DEBadMemoryType::DEBadMemoryType(string device, string memory) {
  _message = string("Memory type \"") + memory + string("\" is not supported by \"") + device + string("\" device.");
...
DEBadMemoryType::DEBadMemoryType(string device, string memory, string operation) {
  _message = operation + string(" is not supported by \"") + device + string("\" device for \"") + memory +
             string("\" memory type");
  _message += string("\nPlease use \"u2prog -h:") + device +
              string("\" to see the list of memory types supported by current device.");
}
