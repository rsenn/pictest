...
void
save(FragBuffer* buffer, const Parameters& params) {
  Image* image = NULL;
  try {
...
void
load(FragBuffer* buffer, const Parameters& params, size_t* start, size_t* end) {
  Image* image = NULL;
  const size_t dataSize = 0x100;
  unsigned char data[dataSize];
  *start = 0xFFFFFFFF;
...
void
getKey(const Parameters& params, const unsigned int argument, XTEA_KEY_T key[4]) {
  Image* image;
  size_t address, size;
  try {
