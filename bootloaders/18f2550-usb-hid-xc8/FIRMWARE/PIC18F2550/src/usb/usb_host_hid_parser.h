...
typedef struct _HID_STRINGITEM {
  ...
  uint16_t
      index; // String index for a String descriptor; allows a string to be associated with a particular item or control
  uint16_t minimum; // Specifies the first string index when assigning a group of sequential strings to controls in an
                    // array or bitmap
  uint16_t maximum; // Specifies the last string index when assigning a group of sequential strings to controls in an
                    // array or bitmap
...
typedef struct _USB_HID_DEVICE_RPT_INFO {
  uint16_t reportPollingRate; // This stores the pollrate for the input report. Application can use this to decide the
                              // rate of transfer
  ...
  uint8_t
      globalsNesting; // On encountering every PUSH item , this is incremented , keep track of current depth of Globals
...
typedef enum {
  HID_ERR = 0,             // No error
  HID_ERR_NotEnoughMemory, // If not enough Heap can be allocated, make sure sufficient dynamic memory is aloocated for
                           // the parser
