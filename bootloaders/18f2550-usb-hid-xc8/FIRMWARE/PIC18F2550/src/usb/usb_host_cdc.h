...
#define EVENT_CDC_COMM_READ_DONE                                                                                       \
  EVENT_CDC_BASE + EVENT_CDC_OFFSET + 2 // A CDC Communication Read transfer has completed
#define EVENT_CDC_COMM_WRITE_DONE                                                                                      \
  EVENT_CDC_BASE + EVENT_CDC_OFFSET + 3 // A CDC Communication Write transfer has completed
...
typedef struct _USB_CDC_ACM_FN_DSC {
  ...
  uint8_t bmCapabilities; // The capabilities that this configuration supports. (A bit value of zero means that the
...
typedef struct _USB_CDC_DEVICE_INFO {
  ...
  uint16_t
      remainingBytes; // Number uint8_ts remaining to be transferrerd in case user data length is more than 64 uint8_ts
...
/*******************************************************************************
  ...
  Remarks:
    None
*******************************************************************************/
uint8_t USBHostCDCTransfer(uint8_t deviceAddress,
                           uint8_t request,
                           uint8_t direction,
