...
const unsigned char _psCharacterArray[] = {
...
USBHostPrinterLanguagePostScript(
  ...
  if(command != USB_PRINTER_ATTACHED) {
    ...
    for(printer = 0; (printer < USB_MAX_PRINTER_DEVICES) && (printerListPostScript[printer].deviceAddress != address);
        printer++)
      ;
  ...
  switch(command) {
    ...
    case USB_PRINTER_ATTACHED:
      for(printer = 0; (printer < USB_MAX_PRINTER_DEVICES) && (printerListPostScript[printer].deviceAddress != 0);
          printer++)
        ;
      if(printer != USB_MAX_PRINTER_DEVICES) {
    ...
    case USB_PRINTER_SET_POSITION:
      ...
      sprintf(buffer,
              ...
              (uint16_t)(size >> 16),
              printerListPostScript[printer].currentHeight - (uint16_t)size);
    ...
    case USB_PRINTER_IMAGE_START:
...

      {
        ...
        sprintf(buffer,
                "gsave %d %d %d %7.2f mul sub translate %d %7.2f mul %d %7.2f mul scale %d %d 1 [%d 0 0 -%d 0 %d] {<",
                info->positionX,
                printerListPostScript[printer].currentHeight - info->positionY,
                info->height,
                (double)info->scale,
                info->width,
                (double)info->scale,
    ...
    case USB_PRINTER_GRAPHICS_LINE:
      ...
      sprintf(buffer,
              COMMAND_SET_POSITION COMMAND_GRAPHICS_LINE_TO COMMAND_STROKE,
              ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sLine.x1,
              printerListPostScript[printer].currentHeight -
                  (((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sLine.y1),
    ...
    case USB_PRINTER_GRAPHICS_LINE_TO:
      ...
      sprintf(buffer,
              COMMAND_GRAPHICS_MOVE_TO COMMAND_GRAPHICS_LINE_TO COMMAND_STROKE,
              printerListPostScript[printer].currentX,
              printerListPostScript[printer].currentHeight - printerListPostScript[printer].currentY,
              (int)(size >> 16),
              printerListPostScript[printer].currentHeight - (int)(size & 0xFFFF));
    ...
    case USB_PRINTER_GRAPHICS_LINE_TO_RELATIVE:
...
#ifdef PRINTER_GRAPHICS_COORDINATE_CHECKING
      if(!X_COORDINATE_IN_RANGE(printerListPostScript[printer].currentX + (int)(size >> 16)) ||
         !Y_COORDINATE_IN_RANGE(printerListPostScript[printer].currentY + (int)(size & 0xFFFF))) {
        return USB_PRINTER_BAD_PARAMETER;
      }
#endif
      ...
      sprintf(buffer,
              COMMAND_GRAPHICS_MOVE_TO COMMAND_GRAPHICS_LINE_TO_RELATIVE COMMAND_STROKE,
              printerListPostScript[printer].currentX,
              printerListPostScript[printer].currentHeight - printerListPostScript[printer].currentY,
              (int)(size >> 16),
              (int)(size & 0xFFFF) * -1);
      _SetCurrentPosition(printer,
                          printerListPostScript[printer].currentX + (int)(size >> 16),
                          printerListPostScript[printer].currentY + (int)(size & 0xFFFF));
      USBHOSTPRINTER_SETFLAG_COPY_DATA(transferFlags);
      return USBHostPrinterWrite(printerListPostScript[printer].deviceAddress, buffer, strlen(buffer), transferFlags);
      break;
    ...
    case USB_PRINTER_GRAPHICS_ARC:
...

      {
        ...
        x = (((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sArc.xR +
             ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sArc.xL) /
            2;
        y = (((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sArc.yT +
             ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sArc.yB) /
            2;

        sprintf(buffer,
                COMMAND_GRAPHICS_MOVE_TO COMMAND_GRAPHICS_ARC COMMAND_FILL,
                x,
                printerListPostScript[printer].currentHeight - y,
                x,
                printerListPostScript[printer].currentHeight - y,
                ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sArc.r2,
                a1,
                a2);
        ...
        if(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sArc.r1 != 0) {
          ...
          sprintf(&(buffer[strlen(buffer)]),
                  COMMAND_GRAPHICS_ARC COMMAND_FILL,
                  x,
                  printerListPostScript[printer].currentHeight - y,
                  ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sArc.r1,
                  a1,
                  a2);
    ...
    case USB_PRINTER_GRAPHICS_BEVEL_FILLED:
      // Note that we must convert the specification to use PostScript's
      // orientation of the Y-axis.
      // xL yB-r moveto xL yT xR yT r arcto clear xR yT xR yB r arcto clear xR yB xL yT r arcto clear xL yB xL yT r
      // arcto clear stroke
    ...
    case USB_PRINTER_GRAPHICS_RECTANGLE_FILLED:
      ...
      // xL yT lineto
      sprintf(&(buffer[strlen(buffer)]),
              COMMAND_GRAPHICS_LINE_TO,
              ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sRectangle.xL,
              printerListPostScript[printer].currentHeight -
                  ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sRectangle.yT);
...
#ifdef PRINTER_GRAPHICS_COORDINATE_CHECKING
    {
      ...
      buffer =
          (char*)USB_MALLOC(15 + (15 * ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sPolygon.numPoints) + 18);
      if(buffer == NULL) {
        return USB_PRINTER_OUT_OF_MEMORY;
      }

      {
        ...
        sprintf(buffer,
                ...
                printerListPostScript[printer].currentHeight -
                    ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sPolygon.points[i + 1]);
        i += 2;
...
bool
USBHostPrinterLanguagePostScriptIsSupported(char* deviceID, USB_PRINTER_FUNCTION_SUPPORT* support) {
...
_PrintFontCommand(uint8_t printer, uint8_t transferFlags) {
  ...
  buffer[0] = '/';
  _SetFontString(printerListPostScript[printer].fontName,
                 printerListPostScript[printer].printerFlags.value,
                 &(buffer[1]));
...
static void
_SetFontString(uint8_t font, uint8_t printerFlags, char* ptr) {
  uint8_t i;
