...
_USBHostHID_Parse_Report(uint8_t* hidReportDescriptor,
                         uint16_t lengthOfDescriptor,
                         uint16_t pollRate,
                         uint8_t interfaceNum) {
  ...
  sizeRequired =
      (sizeof(HID_COLLECTION) * deviceRptInfo.collections) + (sizeof(HID_REPORTITEM) * deviceRptInfo.reportItems) +
      (sizeof(HID_REPORT) * deviceRptInfo.reports) + (sizeof(HID_USAGEITEM) * deviceRptInfo.usages) +
      (sizeof(HID_STRINGITEM) * deviceRptInfo.strings) + (sizeof(HID_DESIGITEM) * deviceRptInfo.designators) +
      (sizeof(int) * deviceRptInfo.maxCollectionNesting) + (sizeof(HID_GLOBALS) * deviceRptInfo.maxGlobalsNesting);
...
***************************************************************************/
static void
_USBHostHID_InitDeviceRptInfo(void) {
...
***************************************************************************/
static void
_USBHostHID_Parse_Collection(HID_ITEM_INFO* ptrItem) {
...
***************************************************************************/
static void
_USBHostHID_Parse_EndCollection(HID_ITEM_INFO* ptrItem) {
  HID_COLLECTION* lcollection;
  uint8_t i;
...
/****************************************************************************
  ...
  Remarks:
    None
***************************************************************************/
static USB_HID_RPT_DESC_ERROR
_USBHostHID_Parse_ReportType(HID_ITEM_INFO* item) {
  ...
  if(deviceRptInfo.globals.logicalMinimum >= ((int32_t)1 << deviceRptInfo.globals.reportsize))
    return (HID_ERR_BadLogicalMin);
  if(deviceRptInfo.globals.logicalMaximum >= ((int32_t)1 << deviceRptInfo.globals.reportsize))
...
***************************************************************************/
static void
_USBHostHID_ConvertDataToSigned(HID_ITEM_INFO* item) {
  uint8_t dataByte = 0;
  uint8_t index;
  ...
  if(index) {
    ...
    if((dataByte & 0x80) != 0) {
      while(index < sizeof(int32_t)) item->Data.sItemData |= (0xFF << ((index++) * 8)); /* extend one */
    }
  }
}
...
***************************************************************************/
bool
USBHostHID_HasUsage(HID_REPORTITEM* reportItem, uint16_t usagePage, uint16_t usage, uint16_t* pindex, uint8_t* count) {
...
#ifdef DEBUG_MODE
void
USBHID_ReportDecriptor_Dump(void) {
  HID_COLLECTION* lcollection;
  HID_REPORT* lreport;
  ...
  for(i = 0; i < deviceRptInfo.reportItems; i++) {
    ...
    UART2PrintString("\r\nDataModes   : ");
    UART2PutHex(lreportItem->dataModes);
    UART2PrintString("\r\n1st Usage   : ");
    ...
    UART2PrintString("\r\n      Globals   ");
    UART2PrintString("\r\nusagePage   : ");
    UART2PutHex(lreportItem->globals.usagePage);
    UART2PrintString("\r\nreportsize   : ");
    UART2PutHex(lreportItem->globals.reportsize);
    UART2PrintString("\r\nreportID   : ");
  ...
  for(i = 0; i < deviceRptInfo.usageItems; i++) {
    ...
    if(itemListPtrs.usageItemList[i].isRange) {
      ...
      UART2PrintString("\r\nUsage: ");
    UART2PutHex(itemListPtrs.usageItemList[i].usage);
  }
}
#endif
