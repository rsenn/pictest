...
#define COMMAND_GRAPHICS_ORIENT_LANDSCAPE                                                                              \
  ESCAPE "*p0x0Y" ESCAPE "*c7920x5760Y" ESCAPE "*c0T" ESCAPE                                                           \
         "%1B;IN;SP1;TR0;SC0,1.411,0,-1.411,2;IR0,100,0,100;" //"RO270;IP;"
#define COMMAND_GRAPHICS_ORIENT_PORTRAIT                                                                               \
  ESCAPE "*p0x0Y" ESCAPE "*c5760x7920Y" ESCAPE "*c0T" ESCAPE                                                           \
         "%1B;IN;SP1;TR0;SC0,1.411,0,-1.411,2;IR0,100,0,100;" //"RO;IP;"
...
#define COMMAND_JOB_START                                                                                              \
  ESCAPE "%-12345X" ESCAPE "E" ESCAPE "&l0E" ESCAPE "&l0F" ESCAPE "&a0L" ESCAPE "&a0M" // ESCAPE "&a0N"
...
const char _pclFontNames[USB_PRINTER_FONT_MAX_FONT][10] = {ESCAPE "(s24607T",  // Avant Garde
...
const char _pclFontNamesVG[USB_PRINTER_FONT_MAX_FONT][4] = {"31", // Avant Garde
                                                            "47", // Bookman
                                                            "3",  // Courier
                                                            "4",  // Helvetica
                                                            "4",  // Helvetica Narrow
...
USBHostPrinterLanguagePCL5(
    uint8_t address, USB_PRINTER_COMMAND command, USB_DATA_POINTER data, uint32_t size, uint8_t transferFlags) {
  char* buffer;
  uint8_t printer = 0;
  ...
  if(command != USB_PRINTER_ATTACHED) {
    ...
    for(printer = 0; (printer < USB_MAX_PRINTER_DEVICES) && (printerListPCL[printer].deviceAddress != address);
        printer++)
      ;
  ...
  if((command > USB_PRINTER_VECTOR_GRAPHICS_START) && (command < USB_PRINTER_VECTOR_GRAPHICS_END) &&
     !USING_VECTOR_GRAPHICS) {
  ...
  switch(command) {
    ...
    case USB_PRINTER_JOB_STOP:
      if(USING_VECTOR_GRAPHICS) {
        return _PrintStaticCommand(address,
                                   COMMAND_GRAPHICS_TERMINATE COMMAND_JOB_STOP,
                                   transferFlags | USB_PRINTER_TRANSFER_NOTIFY);
      } else {
    ...
    case USB_PRINTER_ORIENTATION_PORTRAIT:
      ...
      if(USING_VECTOR_GRAPHICS) {
        return _PrintStaticCommand(address,
                                   COMMAND_GRAPHICS_TERMINATE COMMAND_PORTRAIT COMMAND_GRAPHICS_ORIENT_PORTRAIT,
                                   transferFlags);
      } else {
    ...
    case USB_PRINTER_ORIENTATION_LANDSCAPE:
      ...
      if(USING_VECTOR_GRAPHICS) {
        return _PrintStaticCommand(address,
                                   COMMAND_GRAPHICS_TERMINATE COMMAND_LANDSCAPE COMMAND_GRAPHICS_ORIENT_LANDSCAPE,
                                   transferFlags);
      } else {
    ...
    case USB_PRINTER_EJECT_PAGE:
      ...
      if(USING_VECTOR_GRAPHICS) {
        if(printerListPCL[printer].printerFlags.isLandscape) {
          return _PrintStaticCommand(address,
                                     COMMAND_GRAPHICS_TERMINATE COMMAND_NEXT_PAGE COMMAND_GRAPHICS_ORIENT_LANDSCAPE,
                                     transferFlags);
        } else {
          return _PrintStaticCommand(address,
                                     COMMAND_GRAPHICS_TERMINATE COMMAND_NEXT_PAGE COMMAND_GRAPHICS_ORIENT_PORTRAIT,
                                     transferFlags);
        }
      }
    ...
    case USB_PRINTER_SET_POSITION:
      ...
      sprintf(buffer,
              COMMAND_POSITION_HORIZONTAL COMMAND_POSITION_VERTICAL,
              (uint16_t)(size >> 16) * 10,
              (uint16_t)size * 10);
...

      {
        ...
        if(USING_VECTOR_GRAPHICS) {
          ...
          sprintf(&(buffer[strlen(buffer)]),
                  COMMAND_POSITION_HORIZONTAL COMMAND_POSITION_VERTICAL COMMAND_RASTER_PRESENTATION
                      COMMAND_RASTER_RESOLUTION COMMAND_RASTER_HEIGHT COMMAND_RASTER_WIDTH COMMAND_RASTER_START,
                  info->positionX * 10,
                  info->positionY * 10,
                  resolution,
                  info->height,
    ...
    case USB_PRINTER_IMAGE_STOP:
      if(USING_VECTOR_GRAPHICS) {
        if(printerListPCL[printer].printerFlags.isLandscape) {
          return _PrintStaticCommand(address,
                                     COMMAND_RASTER_END COMMAND_LANDSCAPE COMMAND_GRAPHICS_ORIENT_LANDSCAPE,
                                     transferFlags);
        } else {
          return _PrintStaticCommand(address,
                                     COMMAND_RASTER_END COMMAND_PORTRAIT COMMAND_GRAPHICS_ORIENT_PORTRAIT,
                                     transferFlags);
        }
      } else {
    ...
    case USB_PRINTER_GRAPHICS_FILL_TYPE:
      if((((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sFillType.fillType == PRINTER_FILL_SHADED) ||
        ...
        if(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sFillType.fillType == PRINTER_FILL_SHADED) {
          sprintf(buffer,
                  COMMAND_GRAPHICS_FILL_TYPE_SHADE,
                  ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sFillType.shading);
        } else {
          sprintf(buffer,
                  COMMAND_GRAPHICS_FILL_TYPE_HATCH,
                  ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sFillType.spacing,
    ...
    case USB_PRINTER_GRAPHICS_MOVE_RELATIVE:
#ifdef PRINTER_GRAPHICS_COORDINATE_CHECKING
      if(!X_COORDINATE_IN_RANGE(printerListPCL[printer].currentX + (int)(size >> 16)) ||
         !Y_COORDINATE_IN_RANGE(printerListPCL[printer].currentY + (int)(size & 0xFFFF))) {
        return USB_PRINTER_BAD_PARAMETER;
      }
#endif
      ...
      _SetCurrentPosition(printer,
                          printerListPCL[printer].currentX + (int)(size >> 16),
                          printerListPCL[printer].currentY + (int)(size & 0xFFFF));
      USBHOSTPRINTER_SETFLAG_COPY_DATA(transferFlags);
      return USBHostPrinterWrite(printerListPCL[printer].deviceAddress, buffer, strlen(buffer), transferFlags);
      break;
    ...
    case USB_PRINTER_GRAPHICS_LINE:
#ifdef PRINTER_GRAPHICS_COORDINATE_CHECKING
      if(!X_COORDINATE_IN_RANGE(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sLine.x1) ||
         !Y_COORDINATE_IN_RANGE(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sLine.y1) ||
         !X_COORDINATE_IN_RANGE(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sLine.x2) ||
         !Y_COORDINATE_IN_RANGE(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sLine.y2)) {
        return USB_PRINTER_BAD_PARAMETER;
      }
#endif
      ...
      sprintf(buffer,
              COMMAND_GRAPHICS_LINE COMMAND_GRAPHICS_PEN_UP,
              ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sLine.x1 * 10,
              ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sLine.y1 * 10,
              ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sLine.x2 * 10,
              ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sLine.y2 * 10);
      _SetCurrentPosition(printer,
                          ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sLine.x2,
                          ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sLine.y2);
      USBHOSTPRINTER_SETFLAG_COPY_DATA(transferFlags);
      return USBHostPrinterWrite(printerListPCL[printer].deviceAddress, buffer, strlen(buffer), transferFlags);
      break;
    ...
    case USB_PRINTER_GRAPHICS_LINE_TO:
      ...
      sprintf(buffer,
              COMMAND_GRAPHICS_LINE_TO COMMAND_GRAPHICS_PEN_UP,
              (int)(size >> 16) * 10,
              (int)(size & 0xFFFF) * 10);
    ...
    case USB_PRINTER_GRAPHICS_LINE_TO_RELATIVE:
      ...
      sprintf(buffer,
              COMMAND_GRAPHICS_LINE_TO_RELATIVE COMMAND_GRAPHICS_PEN_UP,
              (int)(size >> 16) * 10,
              (int)(size & 0xFFFF) * 10);
      _SetCurrentPosition(printer,
                          printerListPCL[printer].currentX + (int)(size >> 16),
...
#ifdef PRINTER_GRAPHICS_COORDINATE_CHECKING
      if(!X_COORDINATE_IN_RANGE(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sArc.xL) ||
         !Y_COORDINATE_IN_RANGE(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sArc.yT) ||
         !X_COORDINATE_IN_RANGE(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sArc.xR) ||
         !Y_COORDINATE_IN_RANGE(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sArc.yB)) {
        return USB_PRINTER_BAD_PARAMETER;
      }
#endif
...

      {
        ...
        if(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sArc.octant == 0) {
          ...
          while(!((mask & ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sArc.octant) &&
                  !(maskPrevious & ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sArc.octant))) {
            a1 += 45;
            maskPrevious = mask;
            mask >>= 1;
            if(mask == 0) {
        ...
        sprintf(buffer,
                COMMAND_GRAPHICS_MOVE_TO COMMAND_GRAPHICS_WEDGE,
                (((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sArc.xR +
                 ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sArc.xL) *
                    10 / 2,
                (((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sArc.yT +
                 ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sArc.yB) *
                    10 / 2,
                ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sArc.r2 * 10,
                a1,
                a2);
        ...
        if(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sArc.r1 != 0) {
          ...
          sprintf(&(buffer[strlen(buffer)]),
                  ...
                  (((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sArc.yT +
                   ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sArc.yB) *
                      10 / 2,
                  ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sArc.r1 * 10,
                  a1,
                  a2);
    ...
    case USB_PRINTER_GRAPHICS_CIRCLE:
#ifdef PRINTER_GRAPHICS_COORDINATE_CHECKING
      if(!X_COORDINATE_IN_RANGE(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sCircle.x +
                                ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sCircle.r) ||
         !X_COORDINATE_IN_RANGE(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sCircle.x -
                                ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sCircle.r) ||
         !Y_COORDINATE_IN_RANGE(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sCircle.y +
                                ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sCircle.r) ||
         !Y_COORDINATE_IN_RANGE(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sCircle.y -
                                ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sCircle.r)) {
#ifdef DEBUG_MODE
      ...
      sprintf(buffer,
              COMMAND_GRAPHICS_MOVE_TO COMMAND_GRAPHICS_CIRCLE COMMAND_GRAPHICS_PEN_UP,
              ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sCircle.x * 10,
              ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sCircle.y * 10,
              ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sCircle.r * 10);
      _SetCurrentPosition(printer,
                          ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sCircle.x,
                          ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sCircle.y);
      USBHOSTPRINTER_SETFLAG_COPY_DATA(transferFlags);
      return USBHostPrinterWrite(printerListPCL[printer].deviceAddress, buffer, strlen(buffer), transferFlags);
      break;
    ...
    case USB_PRINTER_GRAPHICS_CIRCLE_FILLED:
      ...
      sprintf(buffer,
              COMMAND_GRAPHICS_MOVE_TO COMMAND_GRAPHICS_ANCHOR_CORNER COMMAND_GRAPHICS_CIRCLE_FILLED
                  COMMAND_GRAPHICS_PEN_UP,
              ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sCircle.x * 10,
              ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sCircle.y * 10,
              ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sCircle.x * 10,
              ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sCircle.y * 10,
    ...
    case USB_PRINTER_GRAPHICS_BEVEL:
#ifdef PRINTER_GRAPHICS_COORDINATE_CHECKING
      if(!X_COORDINATE_IN_RANGE(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sBevel.xL) ||
         !Y_COORDINATE_IN_RANGE(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sBevel.yT) ||
         !X_COORDINATE_IN_RANGE(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sBevel.xR) ||
         !Y_COORDINATE_IN_RANGE(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sBevel.yB)) {
        return USB_PRINTER_BAD_PARAMETER;
      }
#endif
      ...
      sprintf(buffer,
              COMMAND_GRAPHICS_MOVE_TO COMMAND_GRAPHICS_LINE,
              ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sBevel.xL * 10,
              (((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sBevel.yB -
              ...
              (((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sBevel.yB -
               ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sBevel.r) *
                  10,
              ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sBevel.xL * 10,
              (((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sBevel.yT +
               ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sBevel.r) *
                  10);

      // ARr,0,-90;
      sprintf(&(buffer[strlen(buffer)]),
              COMMAND_GRAPHICS_ARC_RELATIVE,
              ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sBevel.r * 10,
              0,
              90);

      // PDxL+r,yT,xR-r,yT;
      sprintf(&(buffer[strlen(buffer)]),
              COMMAND_GRAPHICS_LINE,
      ...
      sprintf(&(buffer[strlen(buffer)]),
              COMMAND_GRAPHICS_ARC_RELATIVE,
              ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sBevel.r * -1 * 10,
              0,
              90);
      ...
      // AR0,-r,-90;PU;
      sprintf(&(buffer[strlen(buffer)]),
              COMMAND_GRAPHICS_ARC_RELATIVE COMMAND_GRAPHICS_PEN_UP,
              0,
              ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sBevel.r * -1 * 10,
              90);
    ...
    case USB_PRINTER_GRAPHICS_BEVEL_FILLED:
      ...
      // PAxL+r,yB; RAxR-r,yT;
      sprintf(buffer,
              COMMAND_GRAPHICS_MOVE_TO COMMAND_GRAPHICS_ANCHOR_CORNER COMMAND_GRAPHICS_BAR,
              (((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sBevel.xL +
               ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sBevel.r) *
                  10,
              ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sBevel.yB * 10,
      ...
      // PAxL,yB-r; RAxR,yT+r;
      sprintf(&(buffer[strlen(buffer)]),
              COMMAND_GRAPHICS_MOVE_TO COMMAND_GRAPHICS_BAR,
              ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sBevel.xL * 10,
              (((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sBevel.yB -
               ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sBevel.r) *
                  10,
    ...
    case USB_PRINTER_GRAPHICS_RECTANGLE:
#ifdef PRINTER_GRAPHICS_COORDINATE_CHECKING
      if(!X_COORDINATE_IN_RANGE(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sRectangle.xL) ||
         !Y_COORDINATE_IN_RANGE(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sRectangle.yT) ||
         !X_COORDINATE_IN_RANGE(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sRectangle.xR) ||
         !Y_COORDINATE_IN_RANGE(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sRectangle.yB)) {
        return USB_PRINTER_BAD_PARAMETER;
      }
#endif
      ...
      sprintf(buffer,
              COMMAND_GRAPHICS_MOVE_TO COMMAND_GRAPHICS_RECTANGLE COMMAND_GRAPHICS_PEN_UP,
      ...
      _SetCurrentPosition(printer,
                          ...
                          ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sRectangle.yB);
      USBHOSTPRINTER_SETFLAG_COPY_DATA(transferFlags);
      return USBHostPrinterWrite(printerListPCL[printer].deviceAddress, buffer, strlen(buffer), transferFlags);
      break;
    ...
    case USB_PRINTER_GRAPHICS_RECTANGLE_FILLED:
      ...
      sprintf(buffer,
              COMMAND_GRAPHICS_MOVE_TO COMMAND_GRAPHICS_ANCHOR_CORNER COMMAND_GRAPHICS_BAR COMMAND_GRAPHICS_PEN_UP,
              ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sRectangle.xL * 10,
              ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sRectangle.yT * 10,
              ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sRectangle.xL * 10,
              ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sRectangle.yT * 10,
...
#ifdef PRINTER_GRAPHICS_COORDINATE_CHECKING
    {
      ...
      for(i = 0; i < ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sPolygon.numPoints * 2; i += 2) {
        if(!X_COORDINATE_IN_RANGE(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sPolygon.points[i]) ||
           !Y_COORDINATE_IN_RANGE(((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sPolygon.points[i + 1])) {
          return USB_PRINTER_BAD_PARAMETER;
        }
      }
    }
      ...
      buffer = (char*)USB_MALLOC(
          12 + (10 * (((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sPolygon.numPoints + 1)) + 4);
      if(buffer == NULL) {
        return USB_PRINTER_OUT_OF_MEMORY;
      }

      {
        ...
        sprintf(&(buffer[strlen(buffer)]),
                "%d,%d;" COMMAND_GRAPHICS_PEN_UP,
                ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sPolygon.points[0] * 10,
                ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sPolygon.points[1] * 10);
      }

      _SetCurrentPosition(printer,
                          ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sPolygon.points[0],
                          ((USB_PRINTER_GRAPHICS_PARAMETERS*)(data.pointerRAM))->sPolygon.points[1]);
      USBHOSTPRINTER_SETFLAG_COPY_DATA(transferFlags);
      return USBHostPrinterWrite(printerListPCL[printer].deviceAddress, buffer, strlen(buffer), transferFlags);
      break;
...
bool
USBHostPrinterLanguagePCL5IsSupported(char* deviceID, USB_PRINTER_FUNCTION_SUPPORT* support) {
...
static uint8_t
_PrintFontCommand(uint8_t printer, uint8_t transferFlags) {
  char* buffer;
  uint8_t font;
...
static uint8_t
_PrintStaticCommand(uint8_t address, char* command, uint8_t transferFlags) {
  char* buffer;
