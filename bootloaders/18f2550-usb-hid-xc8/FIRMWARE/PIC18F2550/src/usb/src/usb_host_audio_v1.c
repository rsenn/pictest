...
uint8_t
USBHostAudioV1ReceiveAudioData(uint8_t deviceAddress, ISOCHRONOUS_DATA* pIsochronousData) {
  uint32_t byteCount;
  uint8_t errorCode;
  uint8_t i;
  ...
  if(!USBHostTransferIsComplete(
         deviceInfoAudioV1[i].ID.deviceAddress, deviceInfoAudioV1[i].endpointAudioStream, &errorCode, &byteCount) ||
  ...
  errorCode = USBHostReadIsochronous(deviceInfoAudioV1[i].ID.deviceAddress,
                                     deviceInfoAudioV1[i].endpointAudioStream,
                                     pIsochronousData);
  if(errorCode) {
  }
...
uint8_t
USBHostAudioV1SetInterfaceFullBandwidth(uint8_t deviceAddress) {
  uint32_t byteCount;
  uint8_t errorCode;
  uint8_t i;
  ...
  errorCode = USBHostIssueDeviceRequest(deviceInfoAudioV1[i].ID.deviceAddress,
                                        0x01,
                                        USB_REQUEST_SET_INTERFACE,
                                        deviceInfoAudioV1[i].settingFullBandwidth,
                                        deviceInfoAudioV1[i].interface,
                                        0,
                                        NULL,
...
USBHostAudioV1SetInterfaceZeroBandwidth(uint8_t deviceAddress) {
  ...
  errorCode = USBHostIssueDeviceRequest(deviceInfoAudioV1[i].ID.deviceAddress,
                                        0x01,
                                        USB_REQUEST_SET_INTERFACE,
                                        deviceInfoAudioV1[i].settingZeroBandwidth,
                                        deviceInfoAudioV1[i].interface,
                                        0,
                                        NULL,
                                        USB_DEVICE_REQUEST_SET,
...
uint8_t
USBHostAudioV1SetSamplingFrequency(uint8_t deviceAddress, uint8_t* frequency) {
  uint8_t errorCode;
  uint8_t i;
...
uint8_t*
USBHostAudioV1SupportedFrequencies(uint8_t deviceAddress) {
  uint8_t i;
...
void
USBHostAudioV1TerminateTransfer(uint8_t deviceAddress) {
  uint8_t i;
...
bool
USBHostAudioV1DataEventHandler(uint8_t address, USB_EVENT event, void* data, uint32_t size) {
...
bool
USBHostAudioV1Initialize(uint8_t address, uint32_t flags, uint8_t clientDriverID) {
  uint8_t* descriptor;
  uint8_t device;
  uint16_t i;
  uint16_t saveIndex;
    ...
    if(descriptor[i + 1] == USB_DESCRIPTOR_INTERFACE) {
      ...
      if((descriptor[i + 5] == USB_CLASS_AUDIO) && (descriptor[i + 6] == USB_SUBCLASS_AUDIO_STREAMING)) {
        ...
        if((deviceInfoAudioV1[device].flags.bfFoundZeroBandwidthSetting == 1) &&
          ...
          USB_HOST_APP_EVENT_HANDLER(deviceInfoAudioV1[device].ID.deviceAddress,
                                     EVENT_AUDIO_ATTACH,
                                     &(deviceInfoAudioV1[device].ID),
                                     sizeof(USB_AUDIO_V1_DEVICE_ID));
...
bool
USBHostAudioV1EventHandler(uint8_t address, USB_EVENT event, void* data, uint32_t size) {
  uint8_t i;
