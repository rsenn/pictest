...
#define _USBHostCDC_SetNextState()                                                                                     \
  { deviceInfoCDC[i].state = (deviceInfoCDC[i].state & STATE_MASK) + NEXT_STATE; }
#define _USBHostCDC_SetNextSubState()                                                                                  \
  { deviceInfoCDC[i].state += NEXT_SUBSTATE; }
...
USB_CDC_DEVICE_INFO deviceInfoCDC[USB_MAX_CDC_DEVICES]
    __attribute__((aligned)); // This structure holds all the information of device collected during enumeration
...
*******************************************************************************/
bool
USBHostCDCInitAddress(uint8_t address, uint32_t flags, uint8_t clientDriverID) {
...
*******************************************************************************/
uint8_t
USBHostCDCDeviceStatus(uint8_t deviceAddress) {
  uint8_t i;
  uint8_t status;
...
*******************************************************************************/
uint8_t
USBHostCDCResetDevice(uint8_t deviceAddress) {
  uint8_t i;
...
#else
  if((deviceInfoCDC[i].state != STATE_DETACHED) && (deviceInfoCDC[i].state != STATE_INITIALIZE_DEVICE))
#endif
  {
      ...
      case STATE_INITIALIZE_DEVICE:
        switch(deviceInfoCDC[i].state & SUBSTATE_MASK) {
          ...
          case SUBSTATE_DEVICE_ENUMERATED:
            USBHostCDCInitAddress(deviceInfoCDC[i].deviceAddress,
                                  0,
                                  deviceInfoCDC[i].clientDriverID); // Initialize device address for application use
          ...
          case SUBSTATE_GET_LINE_CODING:
            ...
            if(!USBHostIssueDeviceRequest(deviceInfoCDC[i].deviceAddress,
                                          ...
                                          (uint8_t*)&CDC_DEV_LINE_CODING_Buffer,
                                          USB_DEVICE_REQUEST_GET,
                                          deviceInfoCDC[i].clientDriverID)) {
              _USBHostCDC_SetNextSubState();
            }
            break;
          ...
          case SUBSTATE_SET_LINE_CODING:
            ...
            if(!USBHostIssueDeviceRequest(deviceInfoCDC[i].deviceAddress,
                                          ...
                                          deviceInfoCDC[i].commInterface.interfaceNum,
                                          USB_CDC_LINE_CODING_LENGTH,
                                          (uint8_t*)PTR_HOST_LINE_CODING_BUFFER,
          ...
          case SUBSTATE_WAIT_FOR_SET_LINE_CODING:
            if(USBHostTransferIsComplete(deviceInfoCDC[i].deviceAddress, 0, &errorCode, &byteCount)) {
              if(errorCode) {
                /* Set error code */
                _USBHostCDC_TerminateTransfer(errorCode);
              } else { // check again if line coding is correctly set
          ...
          case SUBSTATE_SET_CONTROL_LINE_STATE:
            ...
            if(!USBHostIssueDeviceRequest(deviceInfoCDC[i].deviceAddress,
                                          ...
                                          *(uint16_t*)PTR_HOST_CONTROL_LINE_BUFFER,
                                          deviceInfoCDC[i].commInterface.interfaceNum,
        ...
        switch(deviceInfoCDC[i].state & SUBSTATE_MASK) {
          ...
          case SUBSTATE_SEND_READ_REQ:
            ...
            if(deviceInfoCDC[i].endpointDATA == 0x00) {
              errorCode = USBHostIssueDeviceRequest(deviceInfoCDC[i].deviceAddress,
                                                    USB_SETUP_DEVICE_TO_HOST | USB_SETUP_TYPE_CLASS |
                                                        USB_SETUP_RECIPIENT_INTERFACE,
                                                    deviceInfoCDC[i].commRequest,
                                                    0,
                                                    deviceInfoCDC[i].interface,
                                                    deviceInfoCDC[i].reportSize,
                                                    deviceInfoCDC[i].userData,
                                                    USB_DEVICE_REQUEST_GET,
                                                    deviceInfoCDC[i].clientDriverID);
            } else {
              errorCode = USBHostRead(deviceInfoCDC[i].deviceAddress,
                                      deviceInfoCDC[i].endpointDATA,
                                      deviceInfoCDC[i].userData,
                                      deviceInfoCDC[i].reportSize);
            }
          ...
          case SUBSTATE_READ_REQ_WAIT:
            if(USBHostTransferIsComplete(
                   deviceInfoCDC[i].deviceAddress, deviceInfoCDC[i].endpointDATA, &errorCode, &byteCount)) {
              if(errorCode) {
                if(USB_ENDPOINT_STALLED == errorCode) {
                  USBHostClearEndpointErrors(deviceInfoCDC[i].deviceAddress, deviceInfoCDC[i].endpointDATA);
                  deviceInfoCDC[i].returnState = STATE_HOLDING;
          ...
          case SUBSTATE_SEND_WRITE_REQ:
            ...
            if(deviceInfoCDC[i].endpointDATA == 0x00) {
              errorCode = USBHostIssueDeviceRequest(deviceInfoCDC[i].deviceAddress,
                                                    ...
                                                    USB_DEVICE_REQUEST_SET,
                                                    deviceInfoCDC[i].clientDriverID);
            } else { // if transfer size is more than 64 Bytes, multiple transactions are used to transfer the data
              if(deviceInfoCDC[i].remainingBytes > USB_CDC_MAX_PACKET_SIZE) {
                ...
                errorCode = USBHostWrite(deviceInfoCDC[i].deviceAddress,
                                         deviceInfoCDC[i].endpointDATA,
                                         deviceInfoCDC[i].userData,
                                         deviceInfoCDC[i].bytesTransferred);
...
uint8_t
USBHostCDCTransfer(uint8_t deviceAddress,
                   uint8_t request,
                   uint8_t direction,
...
// Make sure the device is in a state ready to read/write.
  ...
  if(!direction) {
    ...
    {
      errorCode =
          USBHostIssueDeviceRequest(deviceInfoCDC[i].deviceAddress,
...
bool
USBHostCDCTransferIsComplete(uint8_t deviceAddress, uint8_t* errorCode, uint8_t* byteCount) {
  uint8_t i;
...
*******************************************************************************/
bool
USBHostCDCEventHandler(uint8_t address, USB_EVENT event, void* data, uint32_t size) {
        ...
        case STATE_READ_REQ_WAIT:
          if(USBHostTransferIsComplete(
            ...
            if(errorCode) {
              ...
              } else { // read request was for data interface
...
*******************************************************************************/
bool
USBHostCDCInitialize(uint8_t address, uint32_t flags, uint8_t clientDriverID) {
...
// Find the device in the table.  If we cannot find it, return an error.
    ...
    if(descriptor[i + 1] == USB_DESCRIPTOR_INTERFACE) {
      ...
      if((descriptor[i + 5] == USB_CDC_COMM_INTF) || (descriptor[i + 5] == 0xFF)) {
        ...
        while(USB_CDC_CS_INTERFACE == descriptor[i + 1]) {
          switch(descriptor[i + 2]) {
            case USB_CDC_DSC_FN_HEADER:
              deviceInfoCDC[device].commInterface.Header_Fn_Dsc = (*((USB_CDC_HEADER_FN_DSC*)&descriptor[i]));
              break;

            case USB_CDC_DSC_FN_CALL_MGT:
              deviceInfoCDC[device].commInterface.Call_Mgt_Fn_Desc = (*((USB_CDC_CALL_MGT_FN_DSC*)&descriptor[i]));
              break;

            case USB_CDC_DSC_FN_ACM:
              deviceInfoCDC[device].commInterface.ACM_Fn_Desc = (*((USB_CDC_ACM_FN_DSC*)&descriptor[i]));
              break;

            case USB_CDC_DSC_FN_UNION:
              deviceInfoCDC[device].commInterface.Union_Fn_Desc = (*((USB_CDC_UNION_FN_DSC*)&descriptor[i]));
              break;
        ...
        if(descriptor[i + 1] == USB_DESCRIPTOR_ENDPOINT) {
          if(descriptor[i + 3] == 0x03) // Communication Interface uses Interrupt endpoint
          {
            if(((descriptor[i + 2] & 0x80) == 0x80) && (deviceInfoCDC[device].commInterface.endpointIN == 0)) {
              ...
              deviceInfoCDC[device].commInterface.endpointMaxDataSize =
                  ((descriptor[i + 4]) | (descriptor[i + 5] << 8));
...

        {
          ...
          while(descriptor[i + 1] == USB_DESCRIPTOR_ENDPOINT) { // Data Interface uses Bulk or Iscochronous endpoint
  ...
  // Send GET Line Coding request to the device
  if(USBHostIssueDeviceRequest(deviceInfoCDC[device].deviceAddress,
                               USB_SETUP_DEVICE_TO_HOST | USB_SETUP_TYPE_CLASS | USB_SETUP_RECIPIENT_INTERFACE,
                               USB_CDC_GET_LINE_CODING,
                               0,
                               deviceInfoCDC[device].commInterface.interfaceNum,
                               USB_CDC_LINE_CODING_LENGTH,
                               (uint8_t*)&CDC_DEV_LINE_CODING_Buffer,
                               USB_DEVICE_REQUEST_GET,
                               deviceInfoCDC[device].clientDriverID)) {
    return false;
  }
  // wait for transfer event for and then decide to alter the settings or not
...
*******************************************************************************/
void
_USBHostCDC_ResetStateJump(uint8_t i) {
#ifdef USB_ENABLE_TRANSFER_EVENT
  uint8_t errorCode;
#endif
...
*******************************************************************************/
void
USBHostCDC_Init_CDC_Buffers(void) {
