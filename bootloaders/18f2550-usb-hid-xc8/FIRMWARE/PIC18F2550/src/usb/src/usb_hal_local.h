...
#define STATUS_MASK (UIR_USB_RST | UIR_UERR | UIR_SOF_TOK | UIR_TOK_DNE | UIR_UIDLE | UIR_STALL)
...
#if((USBHAL_PID_ERR != UEIR_PID_ERR) || (USBHAL_CRC5 != UEIR_CRC5) || (USBHAL_HOST_EOF != UEIR_HOST_EOF) ||            \
    (USBHAL_CRC16 != UEIR_CRC16) || (USBHAL_DFN8 != UEIR_DFN8) || (USBHAL_BTO_ERR != UEIR_BTO_ERR) ||                  \
    (USBHAL_DMA_ERR != UEIR_DMA_ERR) || (USBHAL_BTS_ERR != UEIR_BTS_ERR))
...
#if(((USB_HAL_HANDSHAKE >> 8) != EP_HSHK) || ((USB_HAL_TRANSMIT >> 8) != EP_EP_TX_EN) ||                               \
    ((USB_HAL_RECEIVE >> 8) != EP_EP_RX_EN) || ((USB_HAL_NO_RETRY >> 8) != EP_RETRY_DIS) ||                            \
    ((USB_HAL_ALLOW_HUB >> 8) != EP_HOST_WOHUB))
#error "USBHALSetEpConfiguration must translate control flags"
#endif

#define ERROR_MASK                                                                                                     \
  (UEIR_PID_ERR | UEIR_CRC5 | UEIR_HOST_EOF | UEIR_CRC16 | UEIR_DFN8 | UEIR_BTO_ERR | UEIR_DMA_ERR | UEIR_BTS_ERR)

#define CTRL_MASK (EP_HSHK | EP_EP_TX_EN | EP_EP_RX_EN | EP_RETRY_DIS | EP_HOST_WOHUB)

#define RESISTOR_CTRL_MASK (UOTGCTRL_DM_LOW | UOTGCTRL_DP_LOW | UOTGCTRL_DM_HIGH | UOTGCTRL_DP_HIGH)
