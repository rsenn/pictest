...
UINT8 ROM ASSOCIATION_RESPONSE_REJECTED_PERMANENT[ASSOCIATION_RESPONSE_REJECTED_PERMANENT_SIZE] = {
    0xE3,
    0x00, // APDU CHOICE Type (AareApdu)
    0x00,
    0x2C, // CHOICE.length = 44
    0x00,
    0x01, // result = rejected-permanent
};

/*Release Request APDU */
UINT8 ROM RELEASE_REQUEST[RELEASE_REQUEST_SIZE] = {
    0xE4,
    0x00, /* APDU CHOICE Type (RlrqApdu) */
    0x00,
    0x02, /* CHOICE.length = 2 */
    0x00,
    0x00 /* reason = normal */
};
...
UINT8 ROM RELEASE_RESPONSE[RELEASE_RESPONSE_SIZE] = {
    0xE5,
    0x00, /* APDU CHOICE Type (RlrsApdu) */
    0x00,
    0x02, /* CHOICE.length = 2 */
    0x00,
    0x00 /* reason = normal */
 ...
 *****************************************************************************/
void
PHDAppInit(PHDC_APP_CB callback) {
 ...
 *****************************************************************************/
void
PHDSendAppBufferPointer(UINT8* pAppBuffer) {
 ...
 *****************************************************************************/
void
UsbToPHDComCB(UINT8 USB_Event, void* val) {
  UINT16* size;
  APDU* ptr;
  ...
  switch(apduChoiceType.Val) {
    ...
    default:
      if((PhdComState == PHD_COM_STATE_ASSOCIATING) || (PhdComState == PHD_COM_STATE_ASSOC_CFG_WAITING_APPROVAL) ||
         (PhdComState == PHD_COM_STATE_ASSOC_OPERATING) || (PhdComState == PHD_COM_STATE_DISASSOCIATING)) {
...
PHDDisConnect(void) {
  ...
  } else if((PhdComState == PHD_COM_STATE_DISASSOCIATING) || (PhdComState == PHD_COM_STATE_UNASSOCIATED)) {
    // do nothing
  } else {
 ...
 *****************************************************************************/
static void
PHDAssocRequestHandler(BYTE* apdu_val) {
  if(PhdComState == PHD_COM_STATE_ASSOCIATING) {
    PhdComState = PHD_COM_STATE_UNASSOCIATED;
    USBDevicePHDCSendData(PHDC_BULK_IN_QOS,
                          (UINT8*)ASSOCIATION_RESPONSE_REJECTED_PERMANENT,
                          ASSOCIATION_RESPONSE_REJECTED_PERMANENT_SIZE,
                          MEM_ROM);
    USBDevicePHDCReceiveData(PHDC_BULK_OUT_QOS, pPhdAppBuffer, 0); // get ready to receive
 ...
 *****************************************************************************/
static void
PHDAssocResponseHandler(BYTE* apdu_val) {
  ...
  if((PhdComState == PHD_COM_STATE_ASSOC_OPERATING) || (PhdComState == PHD_COM_STATE_ASSOC_CFG_WAITING_APPROVAL) ||
     (PhdComState == PHD_COM_STATE_DISASSOCIATING)) {
    PHDSendAbortRequestToManager(ABORT_REASON_UNDEFINED);
    return;
  }
 ...
 *****************************************************************************/
static void
PHDPrstApduHandler(BYTE* apdu_val) {
  ...
  if(PhdComState == PHD_COM_STATE_DISASSOCIATING) {
    ...
    PHDSendAbortRequestToManager(ABORT_REASON_UNDEFINED);
  } else if((PhdComState == PHD_COM_STATE_ASSOC_CFG_WAITING_APPROVAL) ||
            (PhdComState == PHD_COM_STATE_ASSOC_CFG_SENDING_CONFIG)) {
    ...
    } else if((data.Val == ROIV_CMIP_EVENT_REPORT_CHOSEN) || (data.Val == ROIV_CMIP_CONFIRMED_EVENT_REPORT_CHOSEN) ||
              (data.Val == ROIV_CMIP_SET_CHOSEN) || (data.Val == ROIV_CMIP_CONFIRMED_SET_CHOSEN) ||
    ...
    if(data.Val == ROIV_CMIP_GET_CHOSEN) // Get command from Manager
    {
      ...
      if(actionType.Val == MDC_ACT_SET_TIME) {
        ...
        pPhdAppBuffer[0] = 0xE7;
        pPhdAppBuffer[1] = 0x00; /* APDU CHOICE Type (AbortApdu) */
        pPhdAppBuffer[2] = 0x00;
        pPhdAppBuffer[3] = 0x0E; /* CHOICE.length = 14 */
        pPhdAppBuffer[4] = 0x00;
        pPhdAppBuffer[5] = 0x0C; /* Octect String Length */
        pPhdAppBuffer[6] = (UINT8)(invoke_id.Val >> 8);
        pPhdAppBuffer[7] = (UINT8)invoke_id.Val;
        pPhdAppBuffer[8] = 0x02;
        pPhdAppBuffer[9] = 0x07; /*rors-cmip-confirmed-action */
        pPhdAppBuffer[10] = 0x00;
        pPhdAppBuffer[11] = 0x06;
        pPhdAppBuffer[12] = 0x00;
        pPhdAppBuffer[13] = 0x00;
        pPhdAppBuffer[14] = 0x0C;
        pPhdAppBuffer[15] = 0x17;
        pPhdAppBuffer[16] = 0x00;
        pPhdAppBuffer[17] = 0x00;
 ...
 *****************************************************************************/
static void
PHDReleaseResponseHandler(BYTE* apdu_val) {
  if((PhdComState == PHD_COM_STATE_ASSOCIATING) || (PhdComState == PHD_COM_STATE_ASSOC_CFG_WAITING_APPROVAL) ||
     (PhdComState == PHD_COM_STATE_DISASSOCIATING) || (PhdComState == PHD_COM_STATE_ASSOC_OPERATING)) {
    PHDSendAbortRequestToManager(ABORT_REASON_UNDEFINED);
  }
}
 ...
 *****************************************************************************/
static void
PHDReleaseRequestHandler(BYTE* apdu_val) {
  if((PhdComState == PHD_COM_STATE_ASSOCIATING)) {
    PHDSendAbortRequestToManager(ABORT_REASON_UNDEFINED);
  ...
  } else if((PhdComState == PHD_COM_STATE_ASSOC_OPERATING) || (PhdComState == PHD_COM_STATE_ASSOC_CFG_SENDING_CONFIG) ||
            (PhdComState == PHD_COM_STATE_ASSOC_CFG_WAITING_APPROVAL)) {
    PhdComState = PHD_COM_STATE_UNASSOCIATED;
    USBDevicePHDCSendData(PHDC_BULK_IN_QOS, (UINT8*)RELEASE_RESPONSE, RELEASE_RESPONSE_SIZE, MEM_ROM);
 ...
 *****************************************************************************/
static void
PHDAbortRequestHandler(BYTE* apdu_val) {
  PhdComState = PHD_COM_STATE_UNASSOCIATED;
  AppCB(PHD_DISCONNECTED);
  PHDDisableAllTimeout();
}
 ...
 *****************************************************************************/
static void
PHDSendAbortRequestToManager(UINT16 abortReason) {
 ...
 *****************************************************************************/
void
PHDSendReleaseRequestToManager(UINT16 releaseReason) {
 ...
 *****************************************************************************/
void
PHDSendRoerMessageToManager(UINT16 invoke_id, UINT16 error_value) {
  pPhdAppBuffer[0] = 0xE7;
  pPhdAppBuffer[1] = 0x00;
  pPhdAppBuffer[2] = 0x00;
  pPhdAppBuffer[3] = 0x0C;
  pPhdAppBuffer[4] = 0x00;
  pPhdAppBuffer[5] = 0x0A;
  pPhdAppBuffer[6] = (UINT8)(invoke_id >> 8);
  pPhdAppBuffer[7] = (UINT8)invoke_id;
  pPhdAppBuffer[8] = 0x03;
  pPhdAppBuffer[9] = 0x00;
  pPhdAppBuffer[10] = 0x00;
  pPhdAppBuffer[11] = 0x04;
  pPhdAppBuffer[12] = (UINT8)(error_value >> 8);
  pPhdAppBuffer[13] = (UINT8)error_value;
  pPhdAppBuffer[14] = 0x00;
  pPhdAppBuffer[15] = 0x00;
 ...
 *****************************************************************************/
static void
PHDMdsAttributesRequestHandler(BYTE* apdu_val) {
