...
USBHostPrinterLanguageESCPOS(
  ...
  if(command != USB_PRINTER_ATTACHED) {
    ...
    for(printer = 0; (printer < USB_MAX_PRINTER_DEVICES) && (printerListESCPOS[printer].deviceAddress != address);
        printer++)
      ;
  ...
  switch(command) {
    ...
    case USB_PRINTER_ATTACHED:
      for(printer = 0; (printer < USB_MAX_PRINTER_DEVICES) && (printerListESCPOS[printer].deviceAddress != 0);
          printer++)
        ;
      if(printer != USB_MAX_PRINTER_DEVICES) {
...
bool
USBHostPrinterLanguageESCPOSIsSupported(char* deviceID, USB_PRINTER_FUNCTION_SUPPORT* support) {
...
USB_DATA_POINTER
USBHostPrinterPOSImageDataFormat(USB_DATA_POINTER image,
...
static uint8_t
_BarcodeCharacterValueCodabar(char c) {
  int i;
...
static uint8_t
_BarcodeCharacterValueCode39(char c) {
  int i;
...
static char
_BarcodeValueCharacterCodabar(uint8_t v) {
  if(v > 19) {
    return '0'; // Error
  }
...
static char
_BarcodeValueCharacterCode39(uint8_t v) {
  if(v > 42) {
    return '0'; // Error
  }
...
static uint8_t
_PrintStaticCommand(uint8_t printer, char* command, uint8_t transferFlags) {
  char* buffer;
