...
// Modify this value to match the VID and PID in your USB device descriptor (in the USB firmware on the
...
[DllImport("setupapi.dll",
           CharSet = CharSet::Seeifdef,
           EntryPoint = "SetupDiGetClassDevs",
           CallingConvention = CallingConvention::Winapi)] extern "C" HDEVINFO
SetupDiGetClassDevsUM(LPGUID ClassGuid,  // Input: Supply the class GUID here.
...
[DllImport("setupapi.dll",
           ...
           CallingConvention = CallingConvention::Winapi)] extern "C" WINSETUPAPI BOOL WINAPI
SetupDiEnumDeviceInterfacesUM(
    ...
    PSP_DEVICE_INTERFACE_DATA
        DeviceInterfaceData); // Output: This function fills in an "SP_DEVICE_INTERFACE_DATA" structure.
...
[DllImport("setupapi.dll",
           ...
           CallingConvention = CallingConvention::Winapi)] extern "C" WINSETUPAPI BOOL WINAPI
SetupDiEnumDeviceInfoUM(HDEVINFO DeviceInfoSet, DWORD MemberIndex, PSP_DEVINFO_DATA DeviceInfoData);
...
[DllImport("setupapi.dll",
           ...
           CallingConvention = CallingConvention::Winapi)] extern "C" WINSETUPAPI BOOL WINAPI
SetupDiGetDeviceRegistryPropertyUM(HDEVINFO DeviceInfoSet,
...
[DllImport("setupapi.dll",
           ...
           CallingConvention = CallingConvention::Winapi)] extern "C" BOOL
SetupDiGetDeviceInterfaceDetailUM(
    ...
    PSP_DEVICE_INTERFACE_DETAIL_DATA
        DeviceInterfaceDetailData,       // Output: Pointer to a strucutre, which will contain the device path.
...
[DllImport("user32.dll",
           ...
           CallingConvention = CallingConvention::Winapi)] extern "C" HDEVNOTIFY WINAPI
RegisterDeviceNotificationUM(HANDLE hRecipient, LPVOID NotificationFilter, DWORD Flags);
...
BOOL AttachedState = FALSE; // Need to keep track of the USB device attachment status for proper plug and play
                            // operation.
...
BOOL ToggleLEDsPending =
    FALSE; // Updated by ToggleLED(s) button click event handler, used by ReadWriteThread (needs to be atomic)
...
public:
  Form1(void) {
    ...
    ANxVoltageToolTip->SetToolTip(
        this->ANxVoltage_lbl,
        "If using a board/PIM without a potentiometer, apply an adjustable voltage to the I/O pin.");
    ANxVoltageToolTip->SetToolTip(
        this->progressBar1,
        "If using a board/PIM without a potentiometer, apply an adjustable voltage to the I/O pin.");
    ToggleLEDToolTip->SetToolTip(this->ToggleLEDs_btn, "Sends a packet of data to the USB device.");
    PushbuttonStateTooltip->SetToolTip(this->PushbuttonState_lbl,
                                       "Try pressing pushbuttons on the USB demo board/PIM.");

    // Register for WM_DEVICECHANGE notifications.  This code uses these messages to detect plug and play
    // connection/disconnection events for USB devices
    ...
    // Now make an initial attempt to find the USB device, if it was already connected to the PC and enumerated prior to
    // launching the application. If it is connected and present, we should open read and write handles to the device so
    // we can communicate with it later. If it was not connected, we will have to wait until the user plugs the device
    // in, and the WM_DEVICECHANGE callback function can process the message and again search for the device.
    if(CheckIfPresentAndGetUSBDevicePath()) // Check and make sure at least one device with matching VID/PID is attached
    {
      ...
      WriteHandleToUSBDevice = CreateFile(DetailedInterfaceDataStructure->DevicePath,
                                          GENERIC_WRITE,
      ...
      ReadHandleToUSBDevice = CreateFile(DetailedInterfaceDataStructure->DevicePath,
                                         GENERIC_READ,
                                         FILE_SHARE_READ | FILE_SHARE_WRITE,
                                         NULL,
                                         OPEN_EXISTING,
      ...
      if((ErrorStatusWrite == ERROR_SUCCESS) && (ErrorStatusRead == ERROR_SUCCESS)) {
        AttachedState = TRUE; // Let the rest of the PC application know the USB device is connected, and it is safe to
                              // read/write to it
        AttachedButBroken = FALSE;
      } else // for some reason the device was physically plugged in, but one or both of the read/write handles didn't
             // open successfully...
      {
...
private:
  System::Windows::Forms::Label ^ ANxVoltage_lbl;

private:
  System::Windows::Forms::Label ^ StatusBox_lbl;

protected:
protected:
private:
  System::Windows::Forms::TextBox ^ StatusBox_txtbx;
...
private:
  System::Windows::Forms::Timer ^ FormUpdateTimer;

private:
  System::Windows::Forms::Button ^ ToggleLEDs_btn;

private:
  System::Windows::Forms::Label ^ PushbuttonState_lbl;

private:
  System::Windows::Forms::ToolTip ^ ANxVoltageToolTip;

private:
  System::Windows::Forms::ToolTip ^ ToggleLEDToolTip;

private:
  System::Windows::Forms::ToolTip ^ PushbuttonStateTooltip;
...
#pragma region Windows Form Designer generated code
  ...
  InitializeComponent(void) {
    ...
    this->ReadWriteThread->DoWork +=
        gcnew System::ComponentModel::DoWorkEventHandler(this, &Form1::ReadWriteThread_DoWork);
    //
    // FormUpdateTimer
    //
  ...
  CheckIfPresentAndGetUSBDevicePath(void) {
    ...
    */
    ...
    while(true) {
      ...
      if(SetupDiEnumDeviceInterfacesUM(
             DeviceInfoTable, NULL, &InterfaceClassGuid, InterfaceIndex, InterfaceDataStructure)) {
        ErrorStatus = GetLastError();
        if(ErrorStatus ==
           ERROR_NO_MORE_ITEMS) // Did we reach the end of the list of matching devices in the DeviceInfoTable?
      ...
      SetupDiGetDeviceRegistryPropertyUM(
          DeviceInfoTable, &DevInfoData, SPDRP_HARDWAREID, &dwRegType, NULL, 0, &dwRegSize);
      ...
      // REG_MULTI_SZ (array of null terminated strings).  To find a device, we only care about the very first string in
      // the buffer, which will be the "device ID".  The device ID is a string which contains the VID and PID, in the
      // example format "Vid_04d8&Pid_003f".
      SetupDiGetDeviceRegistryPropertyUM(
          DeviceInfoTable, &DevInfoData, SPDRP_HARDWAREID, &dwRegType, PropertyValueBuffer, dwRegSize, NULL);
...
// Now check if the first string in the hardware ID matches the device ID of my USB device.
      ...
      free(PropertyValueBuffer); // No longer need the PropertyValueBuffer, free the memory to prevent potential memory
                                 // leaks
      ...
      if(MatchFound == true) {
        // Device must have been found.  Open WinUSB interface handle now.  In order to do this, we will need the actual
        // device path first. We can get the path by calling SetupDiGetDeviceInterfaceDetail(), however, we have to call
        // this function twice:  The first time to get the size of the required structure/buffer to hold the detailed
        // interface data, then a second time to actually get the structure (after we have allocated enough memory for
        // the structure.)
        ...
        SetupDiGetDeviceInterfaceDetailUM(
            DeviceInfoTable, InterfaceDataStructure, DetailedInterfaceDataStructure, StructureSize, NULL, NULL);
  ...
  WndProc(Message % m) override {
    if(m.Msg == WM_DEVICECHANGE) {
      if(((int)m.WParam == DBT_DEVICEARRIVAL) || ((int)m.WParam == DBT_DEVICEREMOVEPENDING) ||
        ...
        // to your device (with known VID/PID) took place before doing any kind of opening or closing of
        // handles/endpoints. (the message could have been totally unrelated to your application/USB device)

        if(CheckIfPresentAndGetUSBDevicePath()) // Check and make sure at least one device with matching VID/PID is
                                                // attached
        {
          ...
          if((AttachedState == FALSE) || (AttachedButBroken == TRUE)) // Check the previous attachment state
          {
            ...
            if((ErrorStatusWrite == ERROR_SUCCESS) && (ErrorStatusRead == ERROR_SUCCESS)) {
              AttachedState = TRUE;
              AttachedButBroken = FALSE;
            } else // For some reason one or both of the read/write pipes couldn't be opened, even though the device was
                   // physically attached.  This could
            { // happen if some other application already opened handles to the device, but did not allow read or write
              // sharing.
              AttachedState =
          ...
          // read/write handles, since the WM_DEVICECHANGE message presumably wasn't caused by our USB device.
        } else // Device must not be connected (or not programmed with correct firmware)
        {
  ...
  FormUpdateTimer_Tick(System::Object ^ sender, System::EventArgs ^ e) {
    ...
    if(AttachedState == TRUE) {
      ...
      if(PushbuttonPressed == FALSE)
        PushbuttonState_lbl->Text = "Pushbutton State: Not Pressed"; // Update the pushbutton state text label on the
                                                                     // form, so the user can see the result
      else
        PushbuttonState_lbl->Text = "Pushbutton State: Pressed"; // Update the pushbutton state text label on the form,
...
private:
  ...
  ReadWriteThread_DoWork(System::Object ^ sender, System::ComponentModel::DoWorkEventArgs ^ e) {
    ...
    const unsigned int _length = 65;
    unsigned char OUTBuffer[_length]; // Allocate a memory buffer equal to the OUT endpoint size + 1
    unsigned char INBuffer[_length];  // Allocate a memory buffer equal to the IN endpoint size + 1
    ...
    while(true) {
      if(AttachedState == TRUE) // Do not try to use the read/write handles unless the USB device is attached and ready
      {
        ...
        // Initialize the rest of the 64-byte packet to "0xFF".  Binary '1' bits do not use as much power, and do not
        // cause as much EMI when they move across the USB cable.  USB traffic is "NRZI" encoded, where '1' bits do not
        // cause the D+/D- signals to toggle states. This initialization is not strictly necessary however.
        for(unsigned char i = 2; i < _length; i++) OUTBuffer[i] = 0xFF;
                     ...
                     0)) // Blocking function, unless an "overlapped" structure is used
        {
          ...
          if(ReadFile(ReadHandleToUSBDevice,
                      &INBuffer,
                      _length,
                      &BytesRead,
                      0)) // Blocking function, unless an "overlapped" structure is used
          {
            ...
            if(INBuffer[1] == 0x37) {
              ADCValue = (INBuffer[3] << 8) +
                         INBuffer[2]; // Need to reformat the data from two unsigned chars into one unsigned int.
            }
          }
        }
        ...
        for(unsigned char i = 2; i < _length;
            i++)               // This loop is not strictly necessary.  Simply initializes unused bytes to
        ...
        if(ToggleLEDsPending == TRUE) {
          OUTBuffer[0] = 0; // The first byte is the "Report ID" and does not get sent over the USB bus.  Always set =
                            // 0.
          ...
          WriteFile(WriteHandleToUSBDevice,
                    &OUTBuffer,
                    _length,
                    &BytesWritten,
                    0); // Blocking function, unless an "overlapped" structure is used
