...
//-------------------------------------------------------BEGIN CUT AND PASTE BLOCK-----------------------------------------------------------------------------------
		...
		while(true)
		{
			if(AttachedState == TRUE)	//Do not try to use the read/write handles unless the USB device is attached and ready
			{
				...
				for (unsigned char i = 2; i <_length; i++)
					OUTBuffer[i] = 0xFF;

				//To get the ADCValue, first, we send a packet with our "READ_POT" command in it.
				if (WriteFile(WriteHandleToUSBDevice, &OUTBuffer, _length, &BytesWritten, 0))	//Blocking function, unless an "overlapped" structure is used
				{
					INBuffer[0] = 0;
					//Now get the response packet from the firmware.
					if (ReadFile(ReadHandleToUSBDevice, &INBuffer, _length, &BytesRead, 0))		//Blocking function, unless an "overlapped" structure is used	
					{
				...
				for (unsigned char i = 2; i <_length; i++)	//This loop is not strictly necessary.  Simply initializes unused bytes to
				...
				if(ToggleLEDsPending == TRUE)
				{
					...
					WriteFile(WriteHandleToUSBDevice, &OUTBuffer, _length, &BytesWritten, 0);	//Blocking function, unless an "overlapped" structure is used
