...
// Section defining the address range to erase for the erase device command, along with the valid programming range to
// be reported by the QUERY_DEVICE command.
#define ProgramMemStart                                                                                                \
  0x001100 // Beginning of application program memory (not occupied by bootloader).  **THIS VALUE MUST BE ALIGNED WITH
           // 64 BYTE BLOCK BOUNDRY** Also, in order to work correctly, make sure the StartPageToErase is set to erase
           // this section.
...
#define QUERY_DEVICE                                                                                                   \
  0x02 // Command that the host uses to learn about the device (what regions can be programmed, and what type of memory
       // is the region)
#define UNLOCK_CONFIG                                                                                                  \
  0x03 // Note, this command is used for both locking and unlocking the config bits (see the "//Unlock Configs Command
       // Definitions" below)
#define ERASE_DEVICE                                                                                                   \
  0x04 // Host sends this command to start an erase operation.  Firmware controls which pages should be erased.
#define PROGRAM_DEVICE                                                                                                 \
  0x05 // If host is going to send a full RequestDataBlockSize to be programmed, it uses this command.
#define PROGRAM_COMPLETE                                                                                               \
  0x06 // If host send less than a RequestDataBlockSize to be programmed, or if it wished to program whatever was left
       // in the buffer, it uses this command.
#define GET_DATA                                                                                                       \
  0x07 // The host sends this command in order to read out memory from the device.  Used during verify (and read/export
       // hex operations)
#define RESET_DEVICE                                                                                                   \
  0x08 // Resets the microcontroller, so it can update the config bits (if they were programmed, and so as to leave the
       // bootloader (and potentially go back into the main application)
...
#define TypeProgramMemory                                                                                              \
  0x01 // When the host sends a QUERY_DEVICE command, need to respond by populating a list of valid memory regions that
       // exist in the device (and should be programmed)
#define TypeEEPROM 0x02
#define TypeConfigWords 0x03
#define TypeEndOfTypeList                                                                                              \
  0xFF // Sort of serves as a "null terminator" like number, which denotes the end of the memory region list has been
       // reached.
...
#define RequestDataBlockSize                                                                                           \
  0x3A // Number of data bytes in a standard request to the PC.  Must be an even number from 2-58 (0x02-0x3A).  Larger
       // numbers make better use of USB bandwidth and yeild shorter program/verify times, but require more
       // micrcontroller RAM for buffer space.
...
ProcessIO(void) {
  ...
  if(BootState == Idle) {
    if(!mHIDRxIsBusy()) // Did we receive a command?
    {
      ...
      for(i = 0; i < TotalPacketSize;
          i++) // Prepare the next packet we will send to the host, by initializing the entire packet to 0x00.
        PacketToPC.Contents[i] = 0; // This saves code space, since we don't have to do it independently in the
                                    // QUERY_DEVICE and GET_DATA cases.
    }
  } else //(BootState must be NotIdle)
  {
    switch(PacketFromPC.Command) {
      case QUERY_DEVICE: {
        ...
        PacketToPC.Type3 = TypeProgramMemory; // Not really program memory (User ID), but may be treated as it it was as
...
#endif
        // Init pad bytes to 0x00...  Already done after we received the QUERY_DEVICE command (just after calling
        // HIDRxReport()).
      ...
      case ERASE_DEVICE: {
        ...
        for(ErasePageTracker = StartPageToErase; ErasePageTracker < (unsigned int)(MaxPageToErase + 1);
            ErasePageTracker++) {
          ClrWdt();
      ...
      case PROGRAM_DEVICE: {
        ...
        {                                    // 0x30 implies config bits
          if(ConfigsLockValue == FALSE) {
            WriteConfigBits(); // Doesn't get reprogrammed if the UNLOCK_CONFIG (LockValue = UNLOCKCONFIG) command
                               // hasn't previously been sent
          }
          BootState = Idle;
          break;
        }
        ...
        if(ProgrammedPointer == (unsigned short long)InvalidAddress) ProgrammedPointer = PacketFromPC.Address;
        ...
        if(ProgrammedPointer == (unsigned short long)PacketFromPC.Address) {
          for(i = 0; i < PacketFromPC.Size; i++) {
            ProgrammingBuffer[BufferedDataIndex] =
                PacketFromPC
                    .Data[i + (RequestDataBlockSize - PacketFromPC.Size)]; // Data field is right justified.  Need to
                                                                           // put it in the buffer left justified.
            BufferedDataIndex++;
            ProgrammedPointer++;
        ...
        // else host sent us a non-contiguous packet address...  to make this firmware simpler, host should not do this
        // without sending a PROGRAM_COMPLETE command in between program sections.
        BootState = Idle;
      } break;
      case PROGRAM_COMPLETE: {
        WriteFlashBlock();
        ProgrammedPointer = InvalidAddress; // Reinitialize pointer to an invalid range, so we know the next
...
#if defined(DEVICE_WITH_EEPROM)
            EEADR = (((unsigned char)PacketFromPC.Address) +
                     i);         // The bits 7:0 are 1:1 mapped to the EEPROM address space values
            EECON1 = 0b00000000; // EEPROM read mode
            EECON1bits.RD = 1;
            PacketToPC.Data[i + ((TotalPacketSize - 6) - PacketFromPC.Size)] = EEDATA;
#endif
          } else // else must have been a normal program memory region, or one that can be read from with the table
                 // pointer
          {
      ...
      case RESET_DEVICE: {
        ...
        for(i = 0; i < 0xFF; i++) {
          ...
          while(WREG) {
            WREG--;
            _asm bra 0 // Equivalent to bra $+2, which takes half as much code as 2 nop instructions
...
void
WriteFlashBlock(void) // Use to write blocks of data to flash.
{
  ...
  CorrectionFactor = (TBLPTRL & 0b00011111); // Correctionfactor = number of bytes tblptr must go back to find the
                                             // immediate preceeding 32 byte boundary
...
#error Double click this error message and fix this section for your microcontroller type.
  ...
  for(i = 0; i < ProgramBlockSize; i++) // Load the programming latches
  {
    if(CorrectionFactor == 0) {
      if(BufferedDataIndex != 0) // If the buffer isn't empty
      {
        ...
        _asm tblwtpostinc _endasm BytesTakenFromBuffer++;
      ...
      TABLAT = 0xFF;
      _asm tblwtpostinc _endasm CorrectionFactor--;
    }
  }
  ...
  for(i = 0; i < BufferedDataIndex;
      i++) // Need to rejustify the remaining data to the "left" of the buffer (if there is any left)
  {
...
void
WriteConfigBits(void) // Also used to write the Device ID
{
  static unsigned char i;
...
UnlockAndActivate(void) {
  ...
  _asm
      ...
      0 MOVLW 0xAA MOVWF EECON2, 0 BSF EECON1, 1,
      0                             // Performs write
      _endasm while(EECON1bits.WR); // Wait until complete (relevant when programming EEPROM, not important when
                                    // programming flash since processor stalls during flash program)
  EECON1bits.WREN = 0; // Good practice now to clear the WREN bit, as further protection against any accidental
                       // activation of self write/erase operations.
}
