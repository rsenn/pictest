...
main(void) {
  ...
  if(sw2 == 1) // This example uses the sw2 I/O pin to determine if the device should enter the bootloader, or the main
               // application code
  {
    ADCON1 = 0x07;   // Restore "reset value" of the ADCON1 register
    _asm goto 0x1100 // If the user is not trying to enter the bootloader, go straight to the main application remapped
                     // "reset" vector.
        _endasm
  }
