...
DB2	EQU	6FH	;DEBUG BYTE 2  -- STATUS_TMP
DB3	EQU	6EH	;DEBUG BYTE 3  -- FSR_TMP
DB4	EQU	6DH	;DEBUG BYTE 4  -- INTCON_TMP
DB5	EQU	6CH	;DEBUG BYTE 5  -- PCLATH_TMP
...
;************** PLACE THE FOLLOWING CODE AFTER THE USER-CODE
 ...
 IF DBRK==1	;DBUG16F: -- DEBUG INSIDER -- TARGET ROUTINE VR 1.2

	ORG	0x800-8		;0800H, 1000H, 1800H, etc
	MOVWF	DB1		;SWITCH ROUTINE
	MOVF	STATUS,W	;SAVE BREAK NUMBER
	CLRF	STATUS		;SET RBANK0
	MOVWF	DB2		;SAVE USER_STATUS
	MOVF	PCLATH,W	;SAVE USER_PCLATH
	PAGESEL	DBUG		;SET DBUG ROM PAGE
	GOTO	DBGX
...
DBGY	BCF	INTCON,GIE	;INT OFF
	MOVWF	DB4		;SAVE USER_INTCON
	MOVF	FSR,W		;SAVE USER_FSR
	MOVWF	DB3
...
DG007	BSF	STATUS,RP0	;SET RBANK1
	...
	GOTO	DG016		;Y, SEND DBN VALUE

;RETURN FROM BREAKPOINT TO USER_MAIN

	MOVF	DB5,W		;BREAK RET
	MOVWF	PCLATH
DG008	MOVF	DB3,W
...
DG009	BTFSC	DB1,4
 ...
 IFDEF	EECON1			;INCLUDE IF EEPROM EXIST
	...
	BANKSEL	EEADR		;SET EE-ADR/DATA BANK
	MOVF	EEDATA,W	;READ INS-EEDATA -> W
	XORWF	DB1
	XORWF	DB1,W		;W <-> DB1
	XORWF	DB1
	MOVWF	EEDATA		;RESTORE USER-EEDATA
	MOVF	FSR,W
...
DG010	MOVF	DB1,W
	...
	CALL	DG017		;ELSE, WAIT INPUT ADR -> DB1
...
DG011	MOVLW	DB2		;SET STATUS_TMP_ADR
	...
	GOTO	DG014
	XORLW	PCLATH^INTCON	;IF ADR=>PCLATH
	BTFSS	STATUS,Z
	GOTO	$+3
	MOVLW	DB5		;SET PCLATH_TMP_ADR
	GOTO	DG014

	XORLW	DB0^PCLATH	;IF ADR=>DB0
	BTFSC	STATUS,Z
	GOTO	DG012		;SKIP DB0

	BTFSS	FSR,0		;INS-ADRH=BANK0?
	BTFSC	DB1,7
	GOTO	DG013		;N, EDIT ANY ADR
	...
	XORLW	DB5^DB4
DG012	MOVLW	DB1
	BTFSS	STATUS,Z	;ADR=DB5?
DG013	MOVF	DB1,W		;INS-ADR -> W
	BTFSC	FSR,0
	BSF	STATUS,IRP	;SET INS-ADRH

DG014	BTFSS	FSR,6		;CMD,6=1?
	GOTO	DG015		;N -> RD CMD, ANY ADR
	MOVWF	FSR		;Y -> WR CMD, SET INS-ADR
	CALL	DG017		;WAIT INPUT INS-DATA
	MOVF	DB1,W
	MOVWF	INDF		;WRITE DATA
	GOTO	DG007		;GOTO INPUT CMD
DG015	MOVWF	FSR		;SET INS-ADR
...
DG017	CLRF	DB1		;INIT DBUG BYTE DB1
	CLRWDT
	BTFSC	X		;WAIT '0' SYNC
	GOTO	$-2
	GOTO	DG019
...
DBUG	MOVWF	DB1		;SHOW BREAK NUMBER
	...
	MOVWF	DB2		;SAVE USER_STATUS
	MOVF	PCLATH,W
DBGX	MOVWF	DB5		;SAVE USER_PCLATH
	MOVF	INTCON,W
	GOTO	DBGY
