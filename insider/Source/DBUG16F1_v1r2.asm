...
;************** PLACE THE FOLLOWING CODE AFTER THE USER-CODE
 ...
 IF DBRK==1	;DBUG16F1: -- DEBUG INSIDER -- TARGET ROUTINE VR 1.2
	...
	ORG	0x800-6		;0800H, 1000H, 1800H, etc
...
DBGY	MOVF	BSR,W		;SAVE USER_BSR
	MOVLB	0		;SET BANK0
	MOVWF	DB6
	MOVF	INTCON,W
	BCF	INTCON,GIE	;INT OFF
	MOVWF	DB7		;SAVE USER_INTCON
	MOVF	FSR0H,W		;SAVE USER_FSR0
	MOVWF	DB5
	MOVF	FSR0L,W
	MOVWF	DB4
...
DG011	MOVF	DB3,W		;BREAK RET
	MOVWF	PCLATH		;RESTORE USER_PCLATH
	MOVF	DB4,W
...
DG015	MOVLW	LOW DB2		;SET STATUS_TMP_ADR
	GOTO	DG016
	XORLW	LOW PCLATH^LOW STATUS	;IF ADR=>PCLATH
	BTFSS	STATUS,Z
	GOTO	$+3
	MOVLW	LOW DB3		;SET FSR0L_TMP_ADR
	GOTO	DG016
	XORLW	LOW FSR0L^LOW PCLATH	;IF ADR=>FSR0L
	BTFSS	STATUS,Z
	GOTO	$+3
	MOVLW	LOW DB4		;SET FSR0L_TMP_ADR
	GOTO	DG016
	XORLW	LOW FSR0H^LOW FSR0L	;IF ADR=>FSR0H
	BTFSS	STATUS,Z
	GOTO	$+3
	MOVLW	LOW DB5		;SET FSR0H_TMP_ADR
	GOTO	DG016
	XORLW	LOW INTCON^LOW FSR0H	;IF ADR=>INTCON
	BTFSS	STATUS,Z
	GOTO	$+3
	MOVLW	LOW DB7		;SET INTCON_TMP_ADR
	GOTO	DG016
...
DG016	CLRF	FSR0H		;POINT BANK0
	...
	BTFSS	STATUS,Z
	XORLW	LOW DB7^LOW DB6		;IF ADR=>DB7
...
DBUG	MOVWF	DB1		;SHOW BREAK NUMBER
	...
	MOVWF	DB2		;SAVE USER_STATUS
	MOVF	PCLATH,W
DBGX	MOVWF	DB3		;SAVE USER_PCLATH
	GOTO	DBGY
