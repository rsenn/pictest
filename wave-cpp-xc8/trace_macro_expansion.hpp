...
std::string
BOOST_WAVE_GETSTRING(std::ostrstream& ss) {
  ss << std::ends;
...
class bad_pragma_exception : public boost::wave::preprocess_exception {
public:
  enum error_code {
  ...
  bad_pragma_exception(
      char const* what_, error_code code, std::size_t line_, std::size_t column_, char const* filename_) throw()
      : boost::wave::preprocess_exception(
            what_, (boost::wave::preprocess_exception::error_code)code, line_, column_, filename_) {}
  ...
  virtual bool
  is_recoverable() const throw() {
    return true;
  }

  virtual int
  get_severity() const throw() {
...
template <typename TokenT> class trace_macro_expansion : public boost::wave::context_policies::eat_whitespace<TokenT> {
...
public:
  trace_macro_expansion(bool preserve_whitespace_,
                        ...
                        std::ostream& tracestrm_,
                        std::ostream& includestrm_,
                        std::ostream& guardstrm_,
                        trace_flags flags_,
                        bool enable_system_command_,
                        bool& generate_output_,
                        std::string const& default_outfile_)
      : outputstrm(output_), tracestrm(tracestrm_), includestrm(includestrm_), guardstrm(guardstrm_), level(0),
        flags(flags_), logging_flags(trace_nothing), enable_system_command(enable_system_command_),
        preserve_whitespace(preserve_whitespace_), preserve_bol_whitespace(preserve_bol_whitespace_),
        generate_output(generate_output_), default_outfile(default_outfile_), emit_relative_filenames(false) {}
  ...
  get_macro_counts() const {
    return counts;
  }

  void
  enable_relative_names_in_line_directives(bool flag) {
    emit_relative_filenames = flag;
  }

  bool
  enable_relative_names_in_line_directives() const {
  ...
  void
  add_noexpandmacro(std::string const& name) {
    noexpandmacros.insert(name);
  }

  void
  set_license_info(std::string const& info) {
    license_info = info;
  }
  ...
  // If the return value is true, the include directive is not executed
  //
  ///////////////////////////////////////////////////////////////////////////
  ...
  found_include_directive(ContextT const& ctx, std::string const& filename, bool include_next) {
    ...
    // Dont include system headers
    if(filename[0] == '<') {
      if(outputstrm.is_open()) {
        outputstrm << "#include " << filename << std::endl;
      } else {
  ...
  void
  expanding_function_like_macro(TokenT const& macrodef,
  ...
  expanding_function_like_macro(ContextT const& ctx,
    ...
    if(0 == get_level()) {
      ...
      stream << macrocall.get_position() << ": " << macrocall.get_value() << "(";
    ...
    // output definition reference
    {
      ...
      stream << macrodef.get_position() << ": see macro definition: " << macrodef.get_value() << "(";
    ...
    if(formal_args.size() > 0) {
      ...
      for(typename std::vector<TokenT>::size_type j = 0; j < formal_args.size(); ++j) {
  ...
  expanding_object_like_macro(ContextT const& ctx,
    ...
    if(0 == get_level()) {
      ...
      stream << macrocall.get_position() << ": " << macrocall.get_value() << std::endl;
      output(BOOST_WAVE_GETSTRING(stream));
      increment_level();
    }
    ...
    // output definition reference
    {
      ...
      stream << macrodef.get_position() << ": see macro definition: " << macrodef.get_value() << std::endl;
      output(BOOST_WAVE_GETSTRING(stream));
    }
    open_trace_body();
  ...
  template <typename ContainerT>
  void
  expanded_macro(ContainerT const& result)
#else
  // new signature
  ...
  template <typename ContainerT>
  void
  rescanned_macro(ContainerT const& result)
#else
  // new signature
  ...
  template <typename ContextT, typename ContainerT>
  bool
  interpret_pragma(ContextT& ctx,
                   ContainerT& pending,
                   typename ContextT::token_type const& option,
                   ContainerT const& valuetokens,
    ...
    if(option.get_value() == "system") {
      if(!enable_system_command) {
        ...
        typename ContextT::string_type msg(boost::wave::util::impl::as_string(values));
        BOOST_WAVE_THROW_CTX(
            ctx, bad_pragma_exception, pragma_system_not_enabled, msg.c_str(), act_token.get_position());
        return false;
      }
    ...
    if(option.get_value() == "stop") {
      ...
      typename ContextT::string_type msg(boost::wave::util::impl::as_string(values));
      BOOST_WAVE_THROW_CTX(
          ctx, boost::wave::preprocess_exception, error_directive, msg.c_str(), act_token.get_position());
      return false;
    }
  ...
  emit_line_directive(ContextT const& ctx, ContainerT& pending, typename ContextT::token_type const& act_token) {
    if(!need_emit_line_directives(ctx.get_language()) || !enable_relative_names_in_line_directives()) {
      return false;
    }
    ...
    std::string file("\"");
    boost::filesystem::path filename(boost::wave::util::create_path(ctx.get_current_relative_filename().c_str()));
  ...
  void
  opened_include_file(std::string const& relname,
  ...
  template <typename ContextT>
  void
  opened_include_file(ContextT const& ctx,
    ...
    if(enabled_include_tracing()) {
      ...
      for(std::size_t i = 0; i < include_depth; ++i) includestrm << " ";
  ...
  template <typename ContextT>
  void
  detected_include_guard(ContextT const& ctx, std::string const& filename, std::string const& include_guard) {
    if(enabled_guard_tracing()) {
      guardstrm << include_guard << ":" << std::endl << "  " << filename << std::endl;
    }
  }
#endif
  ...
  may_skip_whitespace(ContextT const& ctx, TokenT& token, bool& skipped_newline) {
    return this->base_type::may_skip_whitespace(
               ctx, token, need_preserve_comments(ctx.get_language()), preserve_bol_whitespace, skipped_newline)
               ? !preserve_whitespace
               : false;
  }
...
protected:
  ...
  template <typename ContextT, typename ContainerT>
  bool
  interpret_pragma_trace(ContextT& ctx, ContainerT const& values, typename ContextT::token_type const& act_token) {
    typedef typename ContextT::token_type token_type;
    ...
    if(1 == values.size()) {
      ...
      if(value.get_value() == "enable" || value.get_value() == "on" || value.get_value() == "1") {
        // #pragma wave trace(enable)
        enable_tracing(static_cast<trace_flags>(tracing_enabled() | trace_macros));
        valid_option = true;
      } else if(value.get_value() == "disable" || value.get_value() == "off" || value.get_value() == "0") {
  ...
  template <typename ContextT>
  static bool
  interpret_pragma_option_preserve_set(int mode,
    ...
    switch(mode) {
      ...
      case 0:
        ...
        ctx.set_language(enable_preserve_comments(ctx.get_language(), false), false);
        break;
  ...
  bool
  interpret_pragma_option_preserve(ContextT& ctx,
                                   IteratorT& it,
                                   IteratorT end,
                                   typename ContextT::token_type const& act_token) {
    using namespace boost::wave;
    ...
    if(T_IDENTIFIER == id) {
      if((*it).get_value() == "push") {
        ...
        if(preserve_options.empty()) {
          BOOST_WAVE_THROW_CTX(
              ctx, bad_pragma_exception, pragma_mismatched_push_pop, "preserve", act_token.get_position());
        }

        // pop output preserve from the internal option stack
        bool result = interpret_pragma_option_preserve_set(preserve_options.top(),
                                                           preserve_whitespace,
  ...
  template <typename ContextT, typename IteratorT>
  bool
  interpret_pragma_option_line(ContextT& ctx,
                               IteratorT& it,
                               IteratorT end,
                               typename ContextT::token_type const& act_token) {
    using namespace boost::wave;
    ...
    if(T_IDENTIFIER == id) {
      if((*it).get_value() == "push") {
        ...
        if(line_options.empty()) {
          BOOST_WAVE_THROW_CTX(ctx, bad_pragma_exception, pragma_mismatched_push_pop, "line", act_token.get_position());
        }

        // pop output line from the internal option stack
        ctx.set_language(enable_emit_line_directives(ctx.get_language(), 0 != line_options.top()), false);
    ...
    if(0 == emit_lines || 1 == emit_lines || 2 == emit_lines) {
      ...
      ctx.set_language(enable_emit_line_directives(ctx.get_language(), emit_lines), false);
      return true;
    }
    return false;
  }
  ...
  interpret_pragma_option_output_open(boost::filesystem::path& fpath,
    ...
    boost::wave::util::create_directories(boost::wave::util::branch_path(fpath));
  ...
  template <typename ContextT, typename IteratorT>
  bool
  interpret_pragma_option_output(ContextT& ctx,
                                 IteratorT& it,
                                 IteratorT end,
                                 typename ContextT::token_type const& act_token) {
    using namespace boost::wave;
    ...
    if(T_STRINGLIT == id) {
      ...
      fs::path fpath(
          boost::wave::util::create_path(util::impl::unescape_lit(fname.substr(1, fname.size() - 2)).c_str()));
      ...
      if((*it).get_value() == "null") {
        ...
        if(output_options.empty() && current_outfile.empty() && !default_outfile.empty() && default_outfile != "-") {
          current_outfile = boost::wave::util::complete_path(default_outfile, ctx.get_current_directory());
        }

        // push current output option onto the internal option stack
        output_options.push(output_option_type(generate_output, current_outfile));
        result = true;
      } else if((*it).get_value() == "pop") {
        // test for mismatched push/pop #pragmas
        if(output_options.empty()) {
          BOOST_WAVE_THROW_CTX(
              ctx, bad_pragma_exception, pragma_mismatched_push_pop, "output", act_token.get_position());
          return false;
        }
        ...
        if(!current_outfile.empty()) {
          ...
          result = interpret_pragma_option_output_open(current_outfile, ctx, act_token);
        } else {
  ...
  StringT
  unlit(StringT const& str) {
    return str.substr(1, str.size() - 2);
  }

  template <typename StringT>
  StringT
  merge_string_lits(StringT const& lhs, StringT const& rhs) {
    StringT result("\"");
  ...
  template <typename ContextT, typename ContainerT>
  void
  join_adjacent_string_tokens(ContextT& ctx, ContainerT const& values, ContainerT& joined_values) {
    using namespace boost::wave;
    ...
    for(const_iterator it = values.begin(); it != end; ++it) {
      ...
      if(id == T_STRINGLIT) {
        if(!current) {
          ...
          current->set_value(merge_string_lits(current->get_value(), (*it).get_value()));
        }
      } else if(current) {
  ...
  template <typename ContextT, typename ContainerT>
  bool
  interpret_pragma_option(ContextT& ctx, ContainerT const& cvalues, typename ContextT::token_type const& act_token) {
    using namespace boost::wave;
    ...
    for(const_iterator it = values.begin(); it != end; /**/) {
      ...
      if(value.get_value() == "preserve") {
        ...
        valid_option = interpret_pragma_option_preserve(ctx, it, end, act_token);
      } else if(value.get_value() == "line") {
        // #pragma wave option(line: [0|1|2|push|pop])
        valid_option = interpret_pragma_option_line(ctx, it, end, act_token);
      } else if(value.get_value() == "output") {
        // #pragma wave option(output: ["filename"|null|default|push|pop])
        valid_option = interpret_pragma_option_output(ctx, it, end, act_token);
      }
  ...
  template <typename ContextT, typename ContainerT>
  bool
  interpret_pragma_system(ContextT& ctx,
                          ContainerT& pending,
    ...
    typedef boost::wave::iteration_context<ContextT, lexer_type, input_policy_type> iteration_context_type;

    iteration_context_type iter_ctx(ctx, stdout_file.c_str(), act_token.get_position(), ctx.get_language());
    ContainerT pragma;

    for(/**/; iter_ctx.first != iter_ctx.last; ++iter_ctx.first) pragma.push_back(*iter_ctx.first);
  ...
  void
  enable_tracing(trace_flags flags) {
    logging_flags = flags;
  }
  ...
  template <typename StringT>
  void
  output(StringT const& outstr) const {
    indent(get_level());
  ...
  indent(int level) const {
    for(int i = 0; i < level; ++i) tracestrm << "  "; // indent
  }
  ...
  bool
  enabled_macro_tracing() const {
    return (flags & trace_macros) && (logging_flags & trace_macros);
  }

  bool
  enabled_include_tracing() const {
    return (flags & trace_includes);
  }

  bool
  enabled_guard_tracing() const {
    return (flags & trace_guards);
  }

  bool
  enabled_macro_counting() const {
    return logging_flags & trace_macro_counts;
  }

  void
  count_invocation(std::string const& name) {
