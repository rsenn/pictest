...
print_copyright() {
  ...
  for(int i = 0; 0 != copyright[i]; ++i) cout << copyright[i] << endl;
...
void validate(boost::any& v, std::vector<std::string> const& s, cmd_line_utils::include_paths*, long);
}
} // namespace boost
...
// given variables_map
bool
read_config_file_options(std::string const& filename,
  ...
  if(!ifs.is_open()) {
    if(!may_fail) {
      cerr << filename << ": command line warning: config file not found" << endl;
    }
    return false;
  }
  ...
  if(options.size() > 0) {
    ...
    po::store(po::command_line_parser(options).options(desc).style(unix_style).run(), vm);
    po::notify(vm);
  }
  return true;
}
...
std::string
trim_quotes(std::string const& file) {
...
void
validate(boost::any& v, std::vector<std::string> const& s, cmd_line_utils::include_paths*, long) {
...
public:
  auto_stop_watch(std::ostream& outstrm_) : print_time(false), outstrm(outstrm_) {}

  ~auto_stop_watch() {
    if(print_time) {
      outstrm << "Elapsed time: " << this->format_elapsed_time() << std::endl;
    }
  }

  void
  set_print_time(bool print_time_) {
...
get_macro_position(Context& ctx,
  ...
  return ctx.get_macro_definition(name, has_parameters, is_predefined, pos, parameters, definition);
}
...
report_error_message(Exception const& e) {
  ...
  cerr << e.file_name() << ":" << e.line_no() << ":" << e.column_no() << ": " << e.description() << endl;

  // errors count as one
  return (e.get_severity() == boost::wave::util::severity_error ||
          e.get_severity() == boost::wave::util::severity_fatal)
             ? 1
             : 0;
}
  ...
  switch(e.get_errorcode()) {
    case preprocess_exception::macro_redefinition: {
      ...
      if(get_macro_position(ctx, e.get_related_name(), pos)) {
        cerr << pos << ": " << preprocess_exception::severity_text(e.get_severity())
...
load_state(po::variables_map const& vm, Context& ctx) {
  ...
  try {
    ...
    cerr << "wave: error while loading state: " << e.what() << endl;
  } catch(boost::wave::preprocess_exception const& e) {
    cerr << "wave: error while loading state: " << e.description() << endl;
  }
#endif
}
...
save_state(po::variables_map const& vm, Context const& ctx) {
  ...
  try {
    if(vm.count("state") > 0) {
      ...
      if(!ofs.is_open()) {
        cerr << "wave: could not open state file for writing: " << state_file.string() << endl;
        // this is non-fatal
      } else {
        using namespace boost::serialization;
        oarchive oa(ofs);
    ...
    cerr << "wave: error while writing state: " << e.what() << endl;
  }
#endif
}
...
bool
list_macro_names(context_type const& ctx, std::string filename) {
  ...
  if(macronames_file != "-") {
    ...
    boost::wave::util::create_directories(boost::wave::util::branch_path(macronames_file));
    macronames_out.open(macronames_file.string().c_str());
    if(!macronames_out.is_open()) {
      cerr << "wave: could not open file for macro name listing: " << macronames_file.string() << endl;
      return false;
    }
  } else {
  ...
  for(name_iterator it = ctx.macro_names_begin(); it != end; ++it) {
    ...
    if(ctx.get_macro_definition(*it, has_pars, predef, pos, pars, def)) {
      ...
      for(context_type::token_sequence_type::const_iterator dit = def.begin(); dit != dend; ++dit) {
...
bool
list_macro_counts(context_type const& ctx, std::string filename) {
  ...
  if(macrocounts_file != "-") {
    ...
    boost::wave::util::create_directories(boost::wave::util::branch_path(macrocounts_file));
    macrocounts_out.open(macrocounts_file.string().c_str());
    if(!macrocounts_out.is_open()) {
      cerr << "wave: could not open file for macro invocation count listing: " << macrocounts_file.string() << endl;
      return false;
    }
  } else {
  ...
  std::map<std::string, std::size_t> const& counts = ctx.get_hooks().get_macro_counts();

  typedef std::map<std::string, std::size_t>::const_iterator iterator;
  iterator end = counts.end();
  for(iterator it = counts.begin(); it != end; ++it) macrocounts_out << (*it).first << "," << (*it).second << std::endl;
...
std::string
read_entire_file(std::istream& instream) {
  std::string content;
  ...
  copy(std::istream_iterator<char>(instream), std::istream_iterator<char>(), std::inserter(content, content.end()));
#else
  content = std::string(std::istreambuf_iterator<char>(instream.rdbuf()), std::istreambuf_iterator<char>());
#endif
  return content;
}
} // anonymous namespace
...
int
do_actual_work(std::string file_name, std::istream& instream, po::variables_map const& vm, bool input_is_stdin) {
  ...
  try {
    ...
    if(vm.count("traceto")) {
      ...
      fs::path trace_file(boost::wave::util::create_path(vm["traceto"].as<std::string>()));

      if(trace_file != "-") {
        boost::wave::util::create_directories(boost::wave::util::branch_path(trace_file));
        traceout.open(trace_file.string().c_str());
        if(!traceout.is_open()) {
          cerr << "wave: could not open trace file: " << trace_file << endl;
          return -1;
        }
      }
    ...
    if(vm.count("listincludes")) {
      ...
      fs::path includes_file(boost::wave::util::create_path(vm["listincludes"].as<std::string>()));

      if(includes_file != "-") {
        boost::wave::util::create_directories(boost::wave::util::branch_path(includes_file));
        includelistout.open(includes_file.string().c_str());
        if(!includelistout.is_open()) {
          cerr << "wave: could not open include list file: " << includes_file.string() << endl;
          return -1;
        }
      }
    ...
    if((enable_trace & trace_includes) && !includelistout.is_open()) {
      ...
      static_cast<std::basic_ios<char>&>(includelistout).rdbuf(cout.rdbuf());
    }

    // Open the stream where to output the list of included file names
    if(vm.count("listguards")) {
      // try to open the file, where to put the include list
      fs::path listguards_file(boost::wave::util::create_path(vm["listguards"].as<std::string>()));

      if(listguards_file != "-") {
        boost::wave::util::create_directories(boost::wave::util::branch_path(listguards_file));
        listguardsout.open(listguards_file.string().c_str());
        if(!listguardsout.is_open()) {
          cerr << "wave: could not open include guard list file: " << listguards_file.string() << endl;
          return -1;
        }
      }
    ...
    if((enable_trace & trace_guards) && !listguardsout.is_open()) {
      ...
      static_cast<std::basic_ios<char>&>(listguardsout).rdbuf(cout.rdbuf());
    }
    ...
    if(vm.count("preserve")) {
      ...
      switch(preserve) {
        ...
        default:
          cerr << "wave: bogus preserve whitespace option value: " << preserve << ", should be 0, 1, 2, or 3" << endl;
          return -1;
      }
    }
    ...
    if(vm.count("license")) {
      ...
      if(!license_stream.is_open()) {
        cerr << "wave: could not open specified license file: " << license_file << endl;
        return -1;
      }
...
#endif
      ctx.set_language(boost::wave::language_support(boost::wave::support_c99 |
    ...
    if(vm.count("c++11")) {
      ...
      ctx.set_language(boost::wave::language_support(
          boost::wave::support_cpp0x | boost::wave::support_option_convert_trigraphs |
          boost::wave::support_option_long_long | boost::wave::support_option_emit_line_directives
#if BOOST_WAVE_SUPPORT_PRAGMA_ONCE != 0
          | boost::wave::support_option_include_guard_detection
#endif
#if BOOST_WAVE_EMIT_PRAGMA_DIRECTIVES != 0
    ...
    if(vm.count("noguard")) {
      ctx.set_language(boost::wave::enable_include_guard_detection(ctx.get_language(), false));
    }
#endif
    ...
    if(vm.count("line")) {
      ...
      if(0 != lineopt && 1 != lineopt && 2 != lineopt) {
        cerr << "wave: bogus value for --line command line option: " << lineopt << endl;
        return -1;
      }
    ...
    if(vm.count("disambiguate")) {
      ...
      if(0 != disambiguateopt && 1 != disambiguateopt) {
        cerr << "wave: bogus value for --disambiguate command line option: " << disambiguateopt << endl;
        return -1;
      }
      ctx.set_language(boost::wave::enable_insert_whitespace(ctx.get_language(), disambiguateopt != 0));
    }
    ...
    if(vm.count("sysinclude")) {
      ...
      for(vector<std::string>::const_iterator cit = syspaths.begin(); cit != end; ++cit) {
    ...
    if(vm.count("include")) {
      cmd_line_utils::include_paths const& ip = vm["include"].as<cmd_line_utils::include_paths>();
      vector<std::string>::const_iterator end = ip.paths.end();

      for(vector<std::string>::const_iterator cit = ip.paths.begin(); cit != end; ++cit) {
      ...
      for(vector<std::string>::const_iterator syscit = ip.syspaths.begin(); syscit != sysend; ++syscit) {
    ...
    if(vm.count("define")) {
      ...
      for(vector<std::string>::const_iterator cit = macros.begin(); cit != end; ++cit) {
    ...
    if(vm.count("predefine")) {
      vector<std::string> const& predefmacros = vm["predefine"].as<vector<std::string>>();
      vector<std::string>::const_iterator end = predefmacros.end();
      for(vector<std::string>::const_iterator cit = predefmacros.begin(); cit != end; ++cit) {
    ...
    if(vm.count("undefine")) {
      vector<std::string> const& undefmacros = vm["undefine"].as<vector<std::string>>();
      vector<std::string>::const_iterator end = undefmacros.end();
      for(vector<std::string>::const_iterator cit = undefmacros.begin(); cit != end; ++cit) {
    ...
    if(vm.count("noexpand")) {
      vector<std::string> const& noexpandmacros = vm["noexpand"].as<vector<std::string>>();
      vector<std::string>::const_iterator end = noexpandmacros.end();
      for(vector<std::string>::const_iterator cit = noexpandmacros.begin(); cit != end; ++cit) {
    ...
    if(vm.count("nesting")) {
      ...
      if(max_depth < 1 || max_depth > 100000) {
        cerr << "wave: bogus maximal include nesting depth: " << max_depth << endl;
        return -1;
      }
    ...
    if(vm.count("output")) {
      // try to open the file, where to put the preprocessed output
      fs::path out_file(boost::wave::util::create_path(vm["output"].as<std::string>()));
    ...
    if(is_interactive) {
      ...
      ctx.set_language(boost::wave::enable_single_line(ctx.get_language()), false);
    }
    ...
    if(vm.count("forceinclude")) {
      ...
      std::vector<std::string> const& force = vm["forceinclude"].as<std::vector<std::string>>();
      std::vector<std::string>::const_reverse_iterator rend = force.rend();
      for(std::vector<std::string>::const_reverse_iterator cit = force.rbegin(); cit != rend; /**/) {
    ...
    do {
      ...
      do {
        try {
          ...
          // some preprocessing error
          if(is_interactive || boost::wave::cpplexer::is_recoverable(e)) {
...
main(int argc, char* argv[]) {
  ...
  if(!BOOST_WAVE_TEST_CONFIGURATION()) {
    cout << "wave: warning: the library this application was linked against was compiled " << endl
         << "               using a different configuration (see wave_config.hpp)." << endl;
  }
  ...
  try {
    ...
    desc_cmdline.add_options()("help,h", "print out program usage (this message)")("version,v",
                                                                                   "print the version number")(
        "copyright", "print out the copyright statement")("config-file",
    ...
    desc_generic.add_options()("output,o",
                               po::value<std::string>(),
                               "specify a file [arg] to use for output instead of stdout or "
                               "disable output [-]")("autooutput,E",
                                                     "output goes into a file named <input_basename>.i")(
        "license", po::value<std::string>(), "prepend the content of the specified file to each created file")(
        "include,I",
        po::value<cmd_line_utils::include_paths>()->composing(),
        "specify an additional include directory")("sysinclude,S",
                                                   po::value<vector<std::string>>()->composing(),
                                                   "specify an additional system include directory")(
        "forceinclude,F", po::value<std::vector<std::string>>()->composing(), "force inclusion of the given file")(
        "define,D",
        po::value<std::vector<std::string>>()->composing(),
        "specify a macro to define (as macro[=[value]])")("predefine,P",
                                                          po::value<std::vector<std::string>>()->composing(),
                                                          "specify a macro to predefine (as macro[=[value]])")(
        "undefine,U", po::value<std::vector<std::string>>()->composing(), "specify a macro to undefine")
#if BOOST_WAVE_USE_DEPRECIATED_PREPROCESSING_HOOKS == 0
        ("noexpand,N",
         po::value<std::vector<std::string>>()->composing(),
         "specify a macro name, which should not be expanded")
#endif
            ("nesting,n", po::value<int>(), "specify a new maximal include nesting depth");

    po::options_description desc_ext("Extended options (allowed everywhere)");

    desc_ext.add_options()("traceto,t",
                           po::value<std::string>(),
                           "output macro expansion tracing information to a file [arg] "
                           "or to stderr [-]")("timer", "output overall elapsed computing time to stderr")(
        "long_long", "enable long long support in C++ mode")
#if BOOST_WAVE_SUPPORT_VARIADICS_PLACEMARKERS != 0
        ("variadics", "enable certain C99 extensions in C++ mode")("c99", "enable C99 mode (implies --variadics)")
#endif
#if BOOST_WAVE_SUPPORT_CPP0X != 0
            ("c++11", "enable C++11 mode (implies --variadics and --long_long)")
                ...
                ("listincludes,l",
                 ...
                 "list names of included files to a file [arg] or to stdout [-]")(
                    ...
                    "3: all whitespace is preserved")("line,L",
...
#if BOOST_WAVE_SUPPORT_PRAGMA_ONCE != 0
                    ("noguard,G",
                     "disable include guard detection")("listguards,g",
                                                        po::value<std::string>(),
    ...
    po::parsed_options opts(
        po::parse_command_line(argc, argv, desc_overall_cmdline, unix_style, cmd_line_utils::at_option_parser));
    po::variables_map vm;
    ...
    std::remove_copy_if(opts.options.begin(),
                        opts.options.end(),
    ...
    if(arguments.size() > 0 && arguments[0].value[0] != "-") {
      ...
      fs::path input_dir(boost::wave::util::complete_path(boost::wave::util::create_path(arguments[0].value[0])));

      // chop of file name
      input_dir = boost::wave::util::branch_path(boost::wave::util::normalize(input_dir));

      // walk up the hierarchy, trying to find a file wave.cfg
      while(!input_dir.empty()) {
        fs::path filename = input_dir / "wave.cfg";
        if(cmd_line_utils::read_config_file_options(filename.string(), desc_overall_cfgfile, vm, true)) {
    ...
    if(vm.count("config-file")) {
      vector<std::string> const& cfg_files = vm["config-file"].as<vector<std::string>>();
      vector<std::string>::const_iterator end = cfg_files.end();
      for(vector<std::string>::const_iterator cit = cfg_files.begin(); cit != end; ++cit) {
        // parse a single config file and store the results
        cmd_line_utils::read_config_file_options(*cit, desc_overall_cfgfile, vm);
      }
    }

    // ... act as required
    if(vm.count("help")) {
      po::options_description desc_help("Usage: wave [options] [@config-file(s)] [file]");
    ...
    if(0 == arguments.size() || 0 == arguments[0].value.size() || arguments[0].value[0] == "-") {
